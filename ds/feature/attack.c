...
void delete_invader(object ob);

varargs int can_fight(object ob)
{
	object env1,env2,owner,attacker;
	if(!ob) return 0;
   ...
   if(ob->query_temp("invader") && userp(ob) && ( userp(this_object()) ||
      attacker = this_object()->query("creater") ) )
   {
	...
	{
     if( this_object()->query("level") < 5 )
	  {
        if(attacker!=this_object()) tell_object(attacker,"你目前的等級不可以攻擊玩家﹗\n");
	  	  return notify_fail("你目前的等級不可以攻擊玩家﹗\n");
	  }

	  if( ob->query("level")>attacker->query("level")+5 ||
         ob->query("level")<attacker->query("level")-5 )
	  {
	  	  if(attacker!=this_object()) tell_object(attacker,"你不可以攻擊此等級的玩家﹗\n");
	  	  return notify_fail("你不可以攻擊此等級的玩家﹗\n");
	  }
	}
     return 1;
   }         //加這一段是因為要讓 invader 也沒有 peace area   by linjack
	...
	if( userp(ob) && (userp(this_object()) || owner = this_object()->query("creater")) )	//攻擊的一定要設定
	{
	  ...
	  if(ob->query_temp("netdead") && (!ob->query_temp("killer")) && (!ob->query_temp("invader")) )
	  {
	  	if(owner!=this_object()) tell_object(owner,"不能攻擊已經斷線的玩家﹗\n");
	  	return notify_fail("不能攻擊已經斷線的玩家! \n");
	  }

          if( wizardp(ob) && wiz_level(ob) != 6 && !wizardp(owner) ) 
                return notify_fail("目前禁止玩家和巫師進行戰鬥! \n");
          if( !wizardp(ob) && wizardp(owner) && wiz_level(owner) != 6 )
                return notify_fail("巫師請不要攻擊一般玩家! \n");




     if( !ob->query_temp("invader") && !ob->query_temp("killer") )
     {
         if( owner->query_temp("invader") > 1 )
		owner->set_temp("invader",2);
	 else owner->add_temp("invader",1);
		call_out("delete_invader",600,owner);
     }
	}
...
object select_opponent()
{
        ...
        if( objectp(defender=victim->query_temp("defender")) )
        {
...
void delete_invader(object invader)
{
 if(!invader) return ;
 if( invader->query_temp("invader") > 0 )
 {
 	invader->add_temp("invader",-1);
	call_out("delete_invader",600,invader);
 }
 return ;
}
