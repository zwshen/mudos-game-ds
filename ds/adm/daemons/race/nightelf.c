...
void setup_attribute(object ob)
{
	...
	if( undefinedp(my["con"]) ) my["con"] = (my["level"]*(10+my["guild_gifts"][1]))/10+random(my["level"]/5);
	if( undefinedp(my["int"]) ) my["int"] = (my["level"]*(10+my["guild_gifts"][2]))/8+random(my["level"]/5);
	if( undefinedp(my["dex"]) ) my["dex"] = (my["level"]*(10+my["guild_gifts"][3]))/8+random(my["level"]/5);
...
void reset_attribute(object ob)
{
	...
	if(userp(ob))
	{
		...
		my["Merits"]["wit"]=q_int/25+q_dex/40;
