...
// Cominging fixed Channel's changing Bug.
...
// protected string valid_channel = ({
private string *valid_channel = ({
  "gwiz", "twiz", "tw", "ds", "es"
});
...
void incoming_request(mapping info)
{
        ...
        if (info["NAME"]) {
                ...
                if( member_array(lower_case(info["CHANNEL"]), valid_channel) == -1 )
                {
                        set("channel_id", "網路頻道精靈");
                        CHANNEL_D->do_channel(this_object(), "sys", sprintf("接收到不明訊息 from: %s(%s)", info["NAME"], info["HOSTADDRESS"]));
                        log_file("dns_wrongch", sprintf("[From: %s(%s)] %s says on (%s):%s\n", info["NAME"], info["HOSTADDRESS"], info["USRNAME"], info["CHANNEL"], info["MSG"]));
                }
                else CHANNEL_D->do_channel(this_object(), lower_case(info["CHANNEL"]), info["MSG"], info["EMOTE"]);
