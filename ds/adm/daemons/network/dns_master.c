...
string start_message()
{
	return sprintf( "||NAME:%s||VERSION:%s||DRIVER:%s||MUDLIB:%s||ENCODING:%s||HOST:%s||PORT:%d"
		"||PORTUDP:%d||MUDNAME:%s||MUDGROUP:%s||LOCATION:%s||TIME:%s||TCP:%s", Mud_name(),
		MUDLIB_VERSION, __VERSION__, MUDLIB_NAME, CHARACTER_SET, query_host_name(),
		mud_port(), my_port, MUD_FULL_NAME, MUD_GROUP, MUD_LOCATION, ctime(time()), TCP_SERVICE_LEVEL);
}
