...
// #include <daemons.h>
/* No found flock.h , so Disable. =Luky= 
#ifdef FILE_LOCKING
#include <flock.h>
#endif
*/
...
int check_valid_read( string fname, int fd ) {
	...
	CHANNEL_D->do_channel( this_object(), "sys", sprintf("[euid]%s [fname]%s [res]%d",socket_info[ fd ][ USER_NAME ],fname,res));
	seteuid( getuid() );
...
//				  creator, domain, tmp, (xfiles[i])[0]);
			files[i] = sprintf("drwxrwsr-x  %12s  %12s  4096  %12s  %s",
					creator, domain, tmp, (xfiles[i])[0]);
		} else {
			/* file */
...
static void parse_comm( int fd, string str ) {
	string *command, tmp, tmp2,*wizlist;
	mixed *misc;
	int port, i, s;
	...
	switch( lower_case( command[ 0 ] ) )  {
		...
		case "user": // specify user name
			...
			CHECK_CMD(1);
// Luky Modified. 2k/9/17
			wizlist = SECURITY_D->get_wizlist();
			if ( !check_access( command[ 1 ] ) && member_array(command[ 1 ], wizlist) == -1) 
			{
...
"530 Login failed.  Please send your complete E-mail address as password.\n" );
		...
		case "list": // give list files in a directory
			...
			if ( sizeof( command ) > 1 )
				...
				tmp = socket_info[ fd ][ CWD ];
			if ( check_valid_read( tmp, fd ) ) {
				tmp2 = ls( tmp, 0, fd ); // tmp2 = ls( tmp, 0, fd );
				if (tmp2 == "")
					socket_write( fd, "550 No files found.\n");
				else
					data_conn( fd, tmp2, "ls", STRING );
