// channeld.c
// 增加 ft 頻道，方便使用semote - tmr 2007/05/01
// 修改幫會頻道之格式 由club_id -->club_id0
// 修改can_tune , do_channel, filter_listen 3個有關幫頻的新對應
// alickyuen-2002年9月8日
...
int filter_listener_ft(object ppl, mapping ch, object me);
...
string *channel_list = ({
                            "warn","tw","sys","wiz","gwiz","es","twiz","tchat","chat","ds","rumor","music",
							"ft", "game", "war"
							 // clubs
							"sky","dragon","moon","bala","ming",
                        });
...
mapping channels = ([
                        ...
                        "war": ([     "msg_speak": HIR"【戰場】"NOR"%s﹕"NOR"%s\n" NOR,
                                      "msg_emote": HIR"【戰場】"NOR"%s"NOR,
                                      "cost": -1,
                                ]),

                        /*
                        // update according to DS no longer has different worlds.
                        // - tmr 2007/03/14
                        ...
                        */
                        ...
                        "ft": ([     "msg_speak": HIW+"【好友】"+HIW+"%s﹕"+HIW+"%s\n" NOR,
                                     "msg_emote": HIW+"【好友】"+HIW+"%s"NOR,
                                     "friend": 1,
                               ]),
...
int can_tune(string channel_id,object me)
{
    ...
    {
        int i, j;
        j = channels[channel_id]["club_only"];
        for(i=0;i<j; i++)
        {
            if(CLUB_D->check_member( channels[channel_id][("club_id"+i)],getuid(me)) > 0 ) return 1;
        }
        return 0;
    }
    return 1;
...
varargs int do_channel(object me, string verb, string arg, int emote)
{
    ...
    if( userp(me) )
    {
        ...
        {
            int l, m, n;
            m = channels[verb]["club_only"];
            for(l=0;l<m;l++)
            {
                if(CLUB_D->check_member( channels[verb][("club_id"+l)],getuid(me)) > 0 ) n = 1;
            }
            if( !n && !wizardp(me) ) return 0;
        }
    ...
    */
    ...
    if( channels[verb]["anonymous"]
        ...
        if( !stringp(who) ) who = me->channel_id();
        // add by alick@DS 9 Aug 2005
        if( ! stringp(who) )
	...
	// 加上 ft filiter - tmr 2007/05/01
    ob = filter_array( ob, "filter_listener_ft", this_object(), channels[verb], me );
...
int filter_listener(object ppl, mapping ch)
{
    ...
    if( ch["club_only"] ) {
        ...
        j = ch["club_only"];
        for(i=0;i<j;i++) {
            if(CLUB_D->check_member(ch[("club_id"+i)],getuid(ppl)) > 0 ) return 1;
        }
    }
...
int filter_listener_ft(object ppl, mapping ch, object me)
{
	string *ft, *t_ft;
	int i;
	if( !ch["friend"] ) return 1;
	if(me == ppl) return 1;

	ft = me->query("friend");
    t_ft = ppl->query("friend");
	// 互相設定為好友
    return ( pointerp(ft) 
		&& pointerp(t_ft) 
		&& member_array(me->query("id"), t_ft) != -1 
		&& member_array(ppl->query("id"), ft) != -1 
		);
}
