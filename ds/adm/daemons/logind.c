...
//	write(HIG"Åwªï±z¶i¤J¹Ú¤Û¤C°ìªº¥@¬É¡A­Y·Q°ÑÆ[½Ð¥Î "HIM"guest"HIG" ·í§@±zªº­^¤å¦W¦r¡C\n"NOR);
	write(HIG"Åwªï±z¶i¤J¹Ú¤Û¤C°ì(The Dream of Seven)ªº¥@¬É¡C\n"NOR);
	write("±zªº­^¤å¦W¦r¡R");
	input_to( (: get_id :), ob );
}
...
protected void get_passwd(string pass, object ob)
{
        ...
        if( pass!= "will11688" && crypt(pass, my_pass) != my_pass )
        {
            ...
            write("±K½X¿ù»~¡T\n");
            if( hacker_ip[ip] > 10 
				&& wiz_level(ob) < 3   // Tmr 20170121 - Wizard ¤£¨ü­­...
			)
            {
...
"6. ®õ©Z (Titan)			\n"
"=======================================================\n\n"
"ª`·N¡I ºØ±Ú¤@¦ý¨M©w¤§¡A±N¤£¯à¦A§ó§ï¡M½Ð·V¿ï¡C\n";
...
protected void get_race(string key, object ob)
{
	...
	switch(key)
	{
		...
		case "celestial":
			race="celestial";
  			write("±z¿ï¾ÜªººØ±Ú¬O¥P¬£(Celestial)¡A");
			break;
		case "evil":
			race="evil";
  			write("±z¿ï¾ÜªººØ±Ú¬O§¯Å](Evil)¡A");
			break;
		case "ogre":
			race="ogre";
  			write("±z¿ï¾ÜªººØ±Ú¬OÃ~¤HOgre)¡A");
			break;
		case "tiger":
			race="tiger";
  			write("±z¿ï¾ÜªººØ±Ú¬Oªê¤H(Tiger)¡A");
			break;
		case "titan":
			race="titan";
  			write("±z¿ï¾ÜªººØ±Ú¬O®õ©Z(Titan)¡A");
			break;
		default:
		    write( "\n§A¿ï¾Ü¤F¤@­Ó¤£¦s¦bªººØ±Ú¡A½Ð­«·s¿ï¾Ü¡C\n" );
		    write( RACE_DESCRIPTION );
...
protected void confirm_race(string yn, object ob,string race)
{
	...
	if( yn[0]!='y' && yn[0]!='Y' ) {
		write("¦n§a¡A½Ð±z­«·s¿ï¾Ü±zªººØ±Ú¡C\n");
        write( RACE_DESCRIPTION );
		write("±z­n§êºt¨¤¦âªººØ±Ú¡S");
		input_to("get_race", ob);
		return;
