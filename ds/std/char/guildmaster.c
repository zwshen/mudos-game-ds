...
int do_advance_check(object ob, object me ) {
    ...
    {
        // 三階 51~100 級 - tmr
        write(ob->name()+"說: 我已經無法再給你進一步的指導了。\n");
        return 1;
    }
	...
	if( ob->query("adv_class") > 0 ) return 0;
...
int do_advance_level(object ob, object me) {
    ...
    levels = me->query("level");
        ...
        if( levels >= 25 && me->query("adv_class") <= 1  // 只支援到50級
        || levels >= 50 && me->query("adv_class") <= 2  // 只支援到50級
             || levels >= 100 && me->query("adv_class") <= 3  // 100級
            || levels >= 200 )    // 200級
...
int do_advance_attr(object ob, object me, string attr) {
    ...
    if( levels>4 )
    {
         if( me->query("class1") == "adventurer" ) ppl_guildgift = ob->query("guild_gifts");
         else ppl_guildgift = me->query("guild_gifts");
...
int do_skill_train(object ob, object me, string argx)
{
      mapping skills;
    mixed cost1;
        string playerclass, guildclass, *name, bank;
...
int do_guild_join(object ob, object me, string arg)
{
    ...
    int clevel, termnum;
      string playerclass, guildclass, msg, termstr;
...
string sol_class(object ob, int adv)
{
    ...
    if((ob->query("adv_class") == 1 && !adv) || adv == 1)
     ...
     // 2 階以上，以 2 階算
    if( ob->query("adv_class") > 2 )
        return  ob->query("class3");
