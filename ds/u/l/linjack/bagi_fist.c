...
int exert(object me, object target, string arg)
{
	...
	if(me->query_skill_mapped("unarmed")=="bagi_fist")
	{
	  ...
	  if( sk > 49 && random(sk) > random(150) )
	  {
	   ...
	   damage = (sk/2) + me->query_damage()*3 + me->query_skill("horse-steps"); //query_damage()中已經包含str.
	   damage = damage/2 +random(damage/10);
//採用horse-steps 所額外積存的bagi-power 來附加特攻威力。
//捨棄原採用 horse-setps LV 計算方式。
//bagi-power 最滿為1000。（首要條件horse-steps= 100)。 by Tmr
// 改為最滿 100 (skill level). Luky
		...
		if(power>10)
		{
			...
			else  { damage=damage*5/6; pcost=30; } //2次
			me->add_temp("exert_mark/bagi-power",-pcost);
			if(wizardp(me) && me->query("env/debug") )
				tell_object(me,"bagi-power "+ power +" -> "+ me->query_temp("exert_mark/bagi-power") +"\n");
		}
...
//if(damage < 30) damage=30+random(me->query_str()/2);
	   ...
	   if( userp(me) && userp(target) ) me->start_busy(2);
		...
		if(wizardp(me) && me->query("env/debug")) printf("[damage: %d]\n",damage);
	}
	else
	{
         ...
         else
	{
		...
		damage = me->query_damage()+sk/2+type["damage"];
...
// Fix to 100;
		...
		if( userp(me) ) damage = damage * (66+random(10)) / 100;
		damage -= random(c/2);
//不給最低傷害力
//        	if( damage < 1 ) damage = random(20);
			...
			// 增加素質對傷害的影響
		damage = COMBAT_D->Merits_damage(me,target,damage,"bar");
		if( random(COMBAT_D->attack_factor(me,"unarmed")+sk ) > random(-COMBAT_D->dodge_factor(target,"dodge"))+type["dodge"]  && damage > 0 )
		{
