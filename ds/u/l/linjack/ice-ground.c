...
int cast(object me, object target, string arg)
{
 ...
 inn=me->query_int();
 mk=me->query_spell("ice-ground");
 cost=20-(me->query_int()/6);
 if(cost<12) cost = 12;
 if(!me->query("env/ice-ground") || me->query("env/ice-ground") == 0 || me->query("env/ice-ground") > 4)
	return notify_fail("請用 set ice-ground 1 使用寒天凍地訣的第一層法術, 最高可以設到4, 再不懂請help ice-ground.\n");
 ...
 if( (int)me->query("mp") < cost*(me->query("env/ice-ground"))+1
  || (int)me->query("ap") < (cost/2)*(me->query("env/ice-ground"))+1
  || (int)me->query("hp") < cost+1 )
            return notify_fail("你的身體狀況不良。\n");
            
 if( (int)me->query("env/ice-ground") == 1)
 {
 ...
 if( (int)me->query("env/ice-ground") == 2)
 {
 ...
 if( (int)me->query("env/ice-ground") == 3)
 {
 ...
 if( (int)me->query("env/ice-ground") == 4)
 {
   message_vision(CYN"\n$N心神合一，口中唸念有詞，突然一陣好似將要把天地凍結的奇寒凝聚在$N四周圍。\n"NOR,me);
   busy=3;
   me->start_busy(busy); //施法動作時間
...
int delay1(object target,object me,int mk)
{
 ...
 if( me->query_spell("ice-ground") < random(15) )
 {
 ...
 shield=target->query_shield();
    ...
    if(damage>80) damage=80+((damage-80)/5);

 message_vision(CYN"$N突然靈光一閃，口中唸出一句:「"+HIC+"飛雪【落】"+NOR+CYN+"」！\n"NOR,me);
 message_vision(CYN"四周寒氣突然凝聚成細碎飛雪，由上墜下快速向$n撞去！\n\n"NOR,me,target);
 ...
 if(damage > 0)
 {
	...
	if( damage > 99 && random(me->query_int()) > random(target->query_int()) )
	{
		test = random(sizeof(limbs));
		limb = limbs[test];
		message_vision(HIC"\n   $N的"+limb+"因為凍寒刺骨而被凍傷了！\n"NOR,target);
		if(limb == "頭部") target->receive_wound("head",(damage/100));
		if(limb == "腳部") target->receive_wound("foot",(damage/100));
		if(limb == "身體") target->receive_wound("body",(damage/100));
		if(limb == "手部") target->receive_wound("hand",(damage/100));
	}
        if( damage > 200 )
	{
...
/*		if( damage > 100 ) {
 ...
 if( random(40+inn) > 15 )
 {
         me->improve_spell("ice-ground",inn/3+random(inn/3)+1);
 }
...
int delay2(object target,object me,int mk)
{
 ...
 if( environment(me) != environment(target) )           
          return notify_fail("目標離開了你的施法範圍。\n");
 if( me->query_spell("ice-ground") < (random(23)+8) )
 {
  message_vision(CYN"$N"CYN"雖然很努力的集中精力，不過熟練不足，施法失敗了。\n"NOR,me);
  return 1;
 }
 ...
 shield=target->query_shield();
    ...
    damage = (mk*3/2)+(inn*3/2)+(level*3/2);
    damage = damage*2/3 + random(damage/3);
    if(userp(me)) damage=damage+(damage/6)-random(damage/8);
    damage = damage - (shield/3) -random(shield/2);
    if(damage>150) damage=150+((damage-150)/5);

 message_vision(CYN"$N突然靈光一閃，口中唸出一句:「"+HIC+"凍霜【落】"+NOR+CYN+"」！\n"NOR,me);
 message_vision(CYN"四周陰風居然凝為霜寒，由上落下快速向$n撞去！\n\n"NOR,me,target);
...
/*		if( damage > 100 ) {
 ...
 if( random(40+inn) > 22 )
 {
         me->improve_spell("ice-ground",inn/3+random(inn/3)+4);
 }
...
int delay3(object target,object me,int mk)
{
 ...
 if( environment(me) != environment(target) )           
          return notify_fail("目標離開了你的施法範圍。\n");
 if( me->query_spell("ice-ground") < (random(31)+20) )
 {
  message_vision(CYN"$N"CYN"雖然很努力的集中精力，不過熟練不足，施法失敗了。\n"NOR,me);
  return 1;
 }
 ...
 shield=target->query_shield();
    ...
    damage = (mk*3/2)+(inn*2)+(level*2);
    damage = damage*2/3 + random(damage/3);
    if(userp(me)) damage=damage+(damage/6)-random(damage/8);
    damage = damage - (shield/3) -random(shield/3);
    if(damage>220) damage=240+((damage-220)/5);

 message_vision(CYN"$N突然靈光一閃，口中唸出一句:「"+HIC+"寒風刀【落】"+NOR+CYN+"」！\n"NOR,me);
 message_vision(CYN"四周寒氣凝成一片片的冰刀，由上落下快速向$n劈去！\n\n"NOR,me,target);
...
/*		if( damage > 100 ) {
 ...
 if( random(40+inn) > 27 )
 {
         me->improve_spell("ice-ground",inn/2+random(inn/2)+1);
 }
...
int delay4(object target,object me,int mk)
{
 ...
 if( environment(me) != environment(target) )           
          return notify_fail("目標離開了你的施法範圍。\n");
 if( me->query_spell("ice-ground") < (random(41)+40) )
 {
  message_vision(CYN"$N"CYN"雖然很努力的集中精力，不過熟練不足，施法失敗了。\n"NOR,me);
  return 1;
 }
 ...
 shield=target->query_shield();
    ...
    damage = (mk*2)+(inn*2)+(level*2);
    damage = damage*2/3 + random(damage/3);
    if(userp(me)) damage=damage+(damage/6)-random(damage/8);
    damage = damage - (shield/3) -random(shield/3);
    if(damage>295) damage=295+((damage-295)/5);

 message_vision(CYN"$N突然靈光一閃，口中唸出一句:「"+HIC+"寒天凍地【落】"+NOR+CYN+"」！\n"NOR,me);
 message_vision(CYN"四周奇寒突然凝聚成一股曠世寒氣，由上往下速向$n衝去！\n\n"NOR,me,target);
...
/*		if( damage > 100 ) {
 ...
 if( random(40+inn) > 35 )
 {
         me->improve_spell("ice-ground",inn/2+random(inn/2)+5);
 }
...
int delay5(object target,object me,int mk)
{
 ...
 message_vision(CYN"$N突然靈光一閃，口中唸出一句:「"+HIC+"【飄】寒天‧【落】動地‧雙式合招"+NOR+CYN+"」！\n"NOR,me);
