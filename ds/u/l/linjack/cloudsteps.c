...
string *dodge_msg = ({
 "$n大步一躍，一式「平步青雲」閃到$N的斜後方。\n",
 "$n縱身一跳，一招「一躍登天」跳至半天高閃開$N的攻擊。\n",
 "$n腳步放慢，以「幻雲悠悠」不急不徐的速度閃開$N的攻勢。\n",
 "$n忽然加快速度，一招「紫霞靈彩」就讓$N眼花撩亂了。\n",
 "$n身形後移數丈，「柔離飄風」一招使$N的殺招鞭長莫及。\n",
});
...
void delay(object me,int cost)
{
	...
	if(me->query("ap")<10)
	{
        ...
        if( me->is_fighting() ) 
	{
		if( me->query_skill("cloudsteps")>random(250) && me->query_skill("dodge")>70 &&
