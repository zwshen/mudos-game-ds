//godsword 神武劍訣 (改革版--革命就是非常之破壞再進而非常之建設)
#include <ansi.h>
inherit SKILL;
inherit SSERVER;
...
{
        int slv,flv,armor,shield,level,inn,sinn,dd,damage;
        object weapon;
        ...
        if(me->query_skill_mapped("sword")=="godsword")
        {
            ...
            if(me->query("ap")<30) return notify_fail("你目前的內勁不足，無法使用絕招。\n");
            if(me->query("mp")<50) return notify_fail("你目前的法力不足，無法使用絕招。\n");
          ...
          {
             sinn=inn-target->query_int();
             armor=target->query_armor();
             shield=target->query_shield();
             damage=(slv/5)+(flv*5/4)+random(flv/4)+inn+(dd/3);
             damage = damage - random(armor/3) - (shield/2);
             if(sinn>0) damage += random(sinn*3/2);
             ...
             switch( me->query_temp("firepower") ) {
              ...
              case 3:
               ...
               damage = damage+(damage*4/5);
               break;
           }
            if( random(me->query_skill("godsword"))+random(flv) > random(target->query_int())+random(target->query_skill("dodge")) )
            {
             message_vision(HIR"$n躲不過$N猛烈的攻勢，被烈焰給吞沒了！！"NOR,me,target);
             //增加素質對傷害的影響
	     if(userp(me)) damage = damage * (70+random(11)) / 100;
	     damage = damage/2;
        ...
        if(!me->skill_active( "godsword",(: call_other, __FILE__, "delay", me ,6:), 10))
...
void firesword(object me,object target,int damage)
{
     int slv,flv,armor,shield,level,inn,sinn,dd;
     object weapon;                
     weapon=me->query_temp("weapon");
     slv=me->query_skill("godsword")+me->query_skill("sword");
     flv=me->query_spell("god-fire");
     inn=me->query_int();
     dd=me->query_damage();
     sinn=inn-target->query_int();
...
//     if( environment(me) != environment(target) ) return notify_fail("目標離開了你的攻擊範圍。\n");
     ...
     if( random(me->query_skill("godsword"))+random(flv) > random(target->query_int())+random(target->query_skill("dodge")) )
     {
       damage=(flv*3/4)+random(flv/4)+(inn)+(dd/4)+random(dd/4);
       damage -= random(armor/3)+shield;
       if(sinn>0) damage += random(sinn);
       if(damage<15) damage=random(8)+10;
       if(damage>300) damage= 300 + ((damage-300)/2);
       message_vision(HIR"$n躲不過$N猛烈的攻勢，被烈焰給吞沒了！！"NOR,me,target);
       //增加素質對傷害的影響
       damage = damage/2;
