...
int delay(object target,object me,int mk)
{
 ...
 shield=target->query_shield();
    ...
    damage = mk+inn*5/2+random(level/2);
    damage = damage/2 + random(damage/4);
    damage = damage - shield/2 -random(shield/2);
    if(userp(me)) damage=damage * (70+random(11)) / 100;
    if(damage<1) damage=random(inn);
...
message_vision("\n突然$N手上烈焰衝天,祭起一道"HIW"『"HIR"神武真火"HIW"』"NOR"直燒向$n！！"NOR,me,target);
 ...
 if(!me->is_fighting(target)) me->kill_ob(target);
 if(random(8)<1) me->add_busy(1);
 if(mk>84 && random(mk + inn) > 50 + random(50))
 {
...
int delay2(object target,object me,int mk)
{
 ...
 shield=target->query_shield();
    ...
    damage = mk+inn*2+random(level/2);
    damage = damage/4 + random(damage/6);
    damage = damage - shield/3 -random(shield/3);
    if(userp(target)) damage=damage*(70+random(11))/100;
