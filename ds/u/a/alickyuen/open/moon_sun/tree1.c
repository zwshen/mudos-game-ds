...
long);
	...
	set("no_recall",1 );
	set("exits", ([
		"down" : "/open/world1/cominging/area/boo-lin",
	]));
...
int do_climb(string arg)
{
        ...
        me->start_busy(2);
        me->receive_wound("right_arm", 2+random(4));
        me->receive_wound("left_arm", 2+random(4));
        me->receive_wound("right_leg", 2+random(4));
        me->receive_wound("left_leg", 2+random(4));
	me->start_busy(2);
	return 1;
}
...
int do_search(string arg)
{
	...
	if( arg == "tree" || arg == "moon tree" )
	{
		...
		bow = new(__DIR__"bow");
		if( bow->can_move(me) != 1) return notify_fail("你拿不動木弓。\n");
		if( bow->move(me) )
		{
