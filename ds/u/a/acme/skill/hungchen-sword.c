...
mapping *action = ({
        ...
        ([
        "action": "$N使出『紅塵劍法』第八式【猛如虎】，手中$w猛揮，看似亂舞，卻是剛猛，朝$n的$n攻去",
        "damage":                17,
        "dodge" :               -10,
        "parry" :               -15,
        "attact_type":  "bar",
...
int exert(object me, object target, string arg)
{
        ...
        if( me->is_ghost() ) return notify_fail("都死了，還要出來害人？\n");
                ...
                if (me->query_skill("hungchen-sword") < 60)
           ...
           if(!me->can_fight(target)) return me->can_fight(target);
                ...
                me->start_busy(3);
                call_out("superpower",0,me,target,1,damage);
                return 1;
        }
...
void superpower(object me,object target,int z,int damage)
{
       ...
       if( z/2 + random(z*2) > random(100)+1 )
       {
//         damage = me->query_skill("hungchen-sword") + random( me->query_skill("sword"));
//         damage =  damage/2 + (random(damage)/2) + (me->query_damage()/2) + random(me->query_damage()/3);
//         damage = damage*6/7;
//         damage = damage - (target->query_armor()/3) - random(target->query_armor()/3);
//         if(damage>320) damage=320+((damage-320)/4);
//         if(damage<50) damage=40+random(21);
//攻擊力過強，於當於的想法不同，故於1999.12.13修改其攻擊力算法。by Acme

         damage = me->query_skill("sword") * 2 + random(me->query("str"));
         damage -= random(target->query_armor()*2);

         if(damage>300) damage=200+random(100);
         if(damage<50) damage=30+random(40);
