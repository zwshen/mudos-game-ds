...
int do_addin()
{
        ...
        ||  me->query("id")==P4Data["Id"]){
            if (!Playing) message_vision("$N¹ïµÛ¤í¸}°Ú¡ã¡ã¤£°±ªº­ú­ú³Ú³Ú.....¡C\n",me);
       ...
       if (Playing){
                ...
                "¹ïµÛ"+me->name()+"»¡¹D¡GµP§½¥¿¦b¶i¦æ¤¤½Ð¨ì®ÇÃä§¤¤@¤U...¡C\n",me);
                else  message_vision("$N¹ïµÛ§Ú¤]­nª±°Õ¡ã¡ã¤£°±ªº­ú­ú³Ú³Ú.....¡C\n",me);
                return 1;
        }
...
int do_dump(string str,object me)
{
        ...
        if (!Playing){
           ...
           command("say µP§½ÁÙ¨S¶}©l§A®³µP¶Ã¥á!!");
           return 1;
        }
        ...
        tell_room(environment(this_object()),me->name()+"»¡¹D¡G"+do_check_num(str,0)+"\n",me);
        ...
        NowPlayer++;
        NowPlayer%=Play;
        message_vision("$N»¡¹D¡G²{¦b½ü¨ì"+HIY+NowPlayerId[NowPlayer]+NOR+"ºNµP¡C\n",this_object());
        if (Auto_Tin){
        ...
        if (count==(end-16-Goned)){
           Playing=0;
           Count++;
           message_vision("$N»¡¹D¡G¦¹§½"+HIY+"¬y§½"+NOR+"½Ð¦A­«·s¶}©l "+HIR+"start"+NOR+" ¡C\n",this_object());
...
int do_eat(string str)
{
        ...
        if (do_check_eat(num1,num2,LastDump)==0){
           ...
           command("say §A²´ºëªá¤F£«!!³o¼Ë¤]¯à¦Y¡C");
           return 1;
        }
        if (num1==num2){
           command("flop "+me->name());
           command("say ¬Û¦PªºµP¤£¯à¦Y¡C");
           return 1;
        }
        if (me->query("id")==P1Data["Id"]) who=P1Data;
        else if (me->query("id")==P2Data["Id"]) who=P2Data;
 ...
 message_vision("\n$N®³¥X¤F¤@±i"+HIG+do_check_num(num1,0)+NOR+
                      "»P¤@±i"+HIY+do_check_num(num2,0)+NOR+
                      "¦Y¤U¤F"+HIR+do_check_num(LastDump,0)+NOR+"¡C\n\n",me);
        who["Out"]+=num2;
        who["Out"]+=LastDump;
        who["Out"]+=num1;
...
int do_gon(string str,object me)
{
        ...
        if (Touched && NowPlayerId[NowPlayer]==me->query("id")){
           NO_HU=0;
           message_vision("\n$NÁy¤WÅS¥X¨¸´cªº¯º®e»¡¹D¡G·tºbhehe.....\n\n",me);
           who["Mj"]=do_delete_Mj(who["Mj"],str,1);
           for (i=0;i<4;i++) who["Out"]+="xx";
           for (i=0;i<4;i++) who["OutGon"]+=str;
        }else{
           NO_HU=1;
           message_vision("\n$N¤j¥s¡G"+do_check_num(str,0)+"¡ã"+HIW+"¤ì°^¡ã¡ã¡ã"+NOR+"\n\n",me);
...
varargs int do_win(string str,object me)
{
        ...
        if (NO_HU || LastDumpId==me->query("id")) return notify_fail("³o¼Ë¤£¯à­JµP¡C\n");
        if (LastDump==""&&Touched) message_vision("\n$N¤j¥s¡G"+HIW+"¤Ñ¡ã¡ã¡ã­J¡ã¡ã¡ã"+NOR+"\n\n",me);
        else{
             tell_room(environment(this_object()),"\n"+me->name()+"¤j¥s¡G"+HIW+"§Ú¡ã­J¡ã°Õ¡ã"+NOR+"\n\n",me);
...
/////////////
         ...
         if (NowPlayerId[0]==who["Id"]) Count++;
           ...
           command("sigh "+who["Id"]);
           message_vision("$N»¡¹D¡Gª±®a ["+HIR+who["Id"]+HIG+"] ¨¥¥E¡ã ¥j¤ë¡ã¡ã¡ã"+NOR+"¡C\n",this_object());
           Times++;
           Count=0;
           if (Play==4){
              if (!winner=get_mjdata(me->query("id"))) winner=({0,0,0,0});
...
/////////////
        ...
        for (i=0; i<26; i++) Mj+="¢i";
        Mj+="[1;36;46m¢«[0m\n";
        Mj+="[1;36;46m¢i[0;36m¢j    "+HIW+"¤¤[0m¤å¦W¦r  [1m¤w[0mª±§½¼Æ  [1mÁ`[0m¾ÔÂÝ  [1m¦Û[0mºN¦¸¼Æ  [1m©ñ[0mºj¦¸¼Æ[1;36;46m¢j[0m\n";
        for (i=0;i<Play;i++){
            if (Play==4) winner=get_mjdata(NowPlayerId[i]);
            Mj+=sprintf("[1;36;46m¢i[0;36m¢j[1;37m%12s  [33m%8d  [35m%6d  [32m%8d  [34m%8d[1;36;46m¢j[0m\n",
...
int do_touch(string str,object me)
{
        ...
        tell_object(me,"§AºN¨ì¤@±i"+do_check_num(str,0)+"\n");
        if (who["Auto"]=="auto"){
           if (do_check_num(str,1) > 34){
              for (i=0;i<1;i++){
                   who["OutFlower"]+=str;
                   HU_F=1;
                   tell_room(environment(this_object()),me->name()+"»¡¹D¡G"+HIB+"ªáµP"+NOR+"¸Éªá!!¡C\n\n"+me->name()+"©¹«á­±¸É¤F¤@±iµP¡C\n",me);
                   tell_object(me,"§A»¡¹D¡G"+HIB+"ªáµP"+NOR+"¸Éªá!!¡C\n\n§A©¹«á­±¸É¤F¤@±iµP¡C\n");
                   str=List[end..end+1];
                   end-=2;
                   tell_object(me,"§AºN¨ì¤F¤@±i"+do_check_num(str,0)+"\n");
...
int do_pon(string str,object me)
{
        ...
        message_vision("\n$N¤j¥s¡G"+do_check_num(str,0)+"¡ã"+HIW+"¥Û¨Ã¡ã¡ã¡ã"+NOR+"\n\n",me);
        for (i=0;i<Play;i++) {
            if (NowPlayerId[NowPlayer]==me->query("id")) break;
            NowPlayer++;
            NowPlayer%=Play;
...
int do_restart(string str)
{
        ...
        message_vision("$N»¡¹D¡GµP§½­«·s³]©w½Ð¥Î"+HIR+" addin "+NOR+"¥[¤JµP§½¡C\n",this_object());
        NowPlayerId[0]="";
        NowPlayerId[1]="";
        NowPlayerId[2]="";
        NowPlayerId[3]="";
...
"f1" : 0,"f2" : 0,"f3" : 0,"f4" : 0,"f5" : 0,"f6" : 0,"f7" : 0,"f8" : 0
        ...
        if (Play<4) command("say ª±®a¤£¨¬¥|¤H©Ò¥H¤£­p¤À¡C");
        P1Data["Mj"]="";
        P1Data["OutGon"]="";
        P1Data["Out"]="";
        P1Data["OutPon"]="";
...
//        for (i=0;i<sizeof(X);i++) printf("%s=%d ",X[i],MjE_Data[X[i]]);
        ...
        Playing=1;
        message_vision("$N»¡¹D¡G²ø®a"+HIY+NowPlayerId[0]+NOR+"½Ð¶}ªùµP¡C\n",this_object());
        return 1;
}
int do_setmj(string str)
{
        ...
        if (!str){
           numstr=this_object()->name()+"¹ïµÛ§A»¡§A³]©wªº¦³:\n";
...
//§â­^¤å¦rÂà¬°¥N½X or ¤¤¤å flag 1 ´N¬OÂà¬°¥N½X flag 2 Âà¬°1..9
mixed do_check_num(string str,int flag)
{
        int number;
...
void check_flower(mapping who)
{
        ...
        while(i--){
           ...
           if (temp=="f1"||temp=="f2"||temp=="f3"||temp=="f4"||
              ...
              if (Playing){
                 tell_room(environment(this_object()),this_player()->name()+"»¡¹D¡G"+HIB+"ªáµP"+NOR+"¸Éªá!!¡C\n\n"+this_player()->name()+"©¹«á­±¸É¤F¤@±iµP¡C\n",this_player());
...
write(@HELP
    ...
    ¢x        top          top aip,top -p,top -w,top -h,top -d                ¢x
    ¢x                  top -l¬Ý¥þ³¡,-lw,-lh,-ld¡C               ¬d¸ê®Æ        ¢x
...
¡@¡@¨C­J´N¦³©³¥x¢´¡A¦b¥[¤W©Ò±oªº¥x´N¬OÁ`¥x¡A­n¬O¦ÛºN´N¡¯¢²¡A¬Û·í©ó¤­¤@ªººâªk¡C
...
int show_top(string str)
{
        ...
        if (file_size(MJ_DATA)==-1)
           return notify_fail("¸ê®ÆÀÉ¨S¨S¦³§ä¨ì¡C\n");
        ...
        show+=HBWHT+"¢¨\n";
        show+="¢i"+HBRED+"¢j ­^¤å¦W¦r ¢x  ¤¤¤å¦W¦r  ¢x¤wª±§½¼Æ¢x Á`¾ÔÂÝ ¢x¦ÛºN¦¸¼Æ¢x©ñºj¦¸¼Æ¢y"+HBWHT+"¢i\n";
        if (str=="all"){
           for (i=0; i<sizeof(MjData); i+=6){///
               show+="¢i"+HBRED;
               show+=sprintf("¢j%10s¢x%12s¢x%8s¢x%8s¢x%8s¢x%8s¢y",
           ...
           if (check==1){
              ...
              show+=sprintf("¢j%10s¢x%12s¢x%8s¢x%8s¢x%8s¢x%8s¢y",
                            MjData[i],MjData[i+1],MjData[i+2],MjData[i+3],MjData[i+4],MjData[i+5]);
              show+=HBWHT+"¢i\n";
           }else return notify_fail("¨S¦³³o­Ó¤Hªº¸ê®Æ¡C\n");
        }
        show+=HBWHT+"¢«";
        for (i=0; i<34; i++) show+="[47m¡@";
        show+=HBWHT+"¢ª"+NOR+"\n";
