...
void give_tackle()
{
      ...
      switch( i ) {
          ...
          case 3:
            ...
            break;
          case 4:
            say(CYN"市井無賴對"+me->query("name")+"說道: 兄弟果然不簡單, 上次老大給了我一個徽記...\n"NOR);
            obj->set_temp("give_tackle",i+1);
            call_out("give_tackle",2,me);
            break;
          case 5:
            command(CYN"say 也不知道是幹啥勞什子的, 就送給你做紀念吧....:Q"NOR);
            obj->set_temp("give_tackle",i+1);
            call_out("give_tackle",2,me);
            break;
          case 6:
            thing=new(__DIR__"obj/mark");
            thing->move(obj);
     ...
     int typeask(string arg)
{
	...
	{
       message_vision("市井無賴不屑的哼了一聲, 嘴裡喃喃說道: 就是討厭你們這種正人君子...\n",me);
	}
        return 1;
}
