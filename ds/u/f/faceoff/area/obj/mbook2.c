...
void create()
{
        set_name("本草綱目", ({ "medicine book","book" }) );
        if( clonep() )
                set_default_object(__FILE__);
        else
        {
                set("unit", "本");
                set("long", "一本給低階學者學習(learn)的藥典。");
		set("no_give", 1);
        }
        setup();
}
...
int do_read(string arg)
{
        ...
        if( me->query_skill("regs-knowledge") == 40 )
                return notify_fail(CYN"你已經可以掌握這本書的內容了。\n"NOR);
	if( me->query_skill("regs-knowledge") < 19 )
...
void read_book(object me, object obj, int flag)
{
        ...
        if( flag == 1 )
        {
                message_vision(YEL"$N鬆鬆衣帶，就地坐下讀起本草綱目。\n"NOR, me);
        ...
        {
                message_vision(YEL"$N正聚精會神的讀著本草綱目。\n"NOR, me);
                me->receive_damage("mp",10); 
        ...
        {
                message_vision(YEL"$N合起本草綱目，默想剛才讀到的新知。\n"NOR, me);
