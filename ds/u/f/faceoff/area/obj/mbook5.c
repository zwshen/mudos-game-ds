...
void create()
{
        set_name("天梟藥典", ({ "medicine book","book" }) );
        if( clonep() )
                set_default_object(__FILE__);
        else
        {
                set("unit", "本");
                set("long", "一本專供天梟長老學習(learn)的藥典。");
		set("no_give", 1);
        }
        setup();
}
...
int do_read(string arg)
{
        ...
        if( me->query_skill("regs-knowledge") == 100 )
                return notify_fail(CYN"你已經可以掌握這本書的內容了。\n"NOR);
	if( me->query_skill("regs-knowledge") < 79 )
...
void read_book(object me, object obj, int flag)
{
        ...
        if( flag == 1 )
        {
                message_vision(BLU"$N鬆鬆衣帶，就地坐下讀起天梟藥典。\n"NOR, me);
        ...
        {
                message_vision(BLU"$N正聚精會神的讀著天梟藥典。\n"NOR, me);
                me->receive_damage("mp",25); 
                me->improve_skill("regs-knowledge",10);
                me->start_busy(2);
                call_out("read_book", 3, me, obj, ++flag);
        }
        else if( flag == 5 )
        {
                message_vision(BLU"$N合起天梟藥典，默想剛才讀到的新知。\n"NOR, me);
                me->receive_damage("mp",60); 
                me->improve_skill("regs-knowledge",20);
                me->start_busy(2);
                me->delete_temp("read_book");
        }
