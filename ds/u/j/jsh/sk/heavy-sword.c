...
int exert(object me, object target, string arg)
{
        ...
        if( me->is_ghost() ) return notify_fail("Pray Luky吧...\n");
...
if(!me->is_fighting(target) ) return notify_fail("你並沒有在戰鬥唷。\n");
        ...
        if(!me->skill_active( "heavy-sword",(: call_other, __FILE__, "delay", me ,5:), 10)) return notify_fail("你的動作還沒完成。\n");

        me->map_skill("sword", "heavy-sword");
        me->reset_action();
        message_vision("\n"HIC"$N以"+weapon->query("name")+HIW "刮起一道旋風，「 鏗 !」的一聲，將其插入地上，引導「重劍術」的真諦。\n"NOR,me);
...
void superpower(object me,object target,int z,int damage)
{
      ...
      if(!me->is_fighting(target)) me->kill_ob(target);
       ...
       if( z/3 + random(z) > random(target->query_skill("dodge")) )
       {
         ...
         if(damage>235) damage=235+((damage-235)/10);
         if(damage<30) damage=25+random(18);
