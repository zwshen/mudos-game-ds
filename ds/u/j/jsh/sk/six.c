...
int exert(string arg)
{
                ...
                int sk=me->query_skill("six_open"),damage,c;
                ...
                damage=me->query("str")*2+me->query_Merits("bar")*10+me->query_skill("six_open")*5-c+random(50);
...
NOR,me,target);
                                target->receive_damage("hp",50+damage);
                                target->start_busy(2); 
                                me->start_busy(1);   
                                me->receive_damage("ap",40+random(5),me);                                  me->add_temp("do_six_open",1); 
                                tell_object(me,HIR"["+damage+"]的傷害值\n"NOR);
                                //target->add_temp("do_six_open",1); 
                                call_out("do_six_opened",10,target);  
                                me->improve_skill("six_open",10+random(me->query_int()/6+sk/8));   
                } else {
                        switch(random(3))
                        {
                        case 0:
                                message_vision(HIW ""+target->query("name")+"在千釣一髮之際，閃開了$N猛烈的攻擊....\n" NOR,target,me); 
                                me->start_busy(2);
                                break;
                        case 1:
