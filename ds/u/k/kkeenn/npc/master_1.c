...
int do_advance(string arg)
{
        ...
        else return notify_fail("§A¤£¬O±¡«°¦Àªº§Ì¤l¡C\n");
}
...
if ( ob->query("quest/win") > 1000 )
      {  
        ...
        ob->add("bank/past",100000);
        ob->set("guild_rank",HIW"¡i"HIR"¦å¤â±¡"HIC"¦B¤ß«°"HIW"¡j"NOR);
        message_vision(HIW"$N¦]¬°³q¹L±¡«°¦Àªº¦ÒÅç¡A©Ò¥HÀò±o10ÂIÁn±æ¤Î¤Q¸U¤¸¥j¹ô\n"NOR,ob);
                ::die();
                return;
        }
message("world:world1:vision",
HIR"\n  ¡mªZªL¬õ©«¡n"HIM":\n\n\t±¡«°¦À¤@¬£³Q"+ob->name()+HIM"À»±Ñ¡A¦b¦¿´ò¤W³Q°£¦W¤F¡I\n\n"+NOR
        ...
        message_vision(HIW"$N¦]¬°·À¤F±¡«°¦À¤@¬£¡A©Ò¥HÀò±o10ÂIÁn±æ¤Î¤Q¸U¤¸¥j¹ô\n"NOR,ob);
                ::die();
                return;
        }
...
int do_guard(string arg)
{
      ...
      command("sorry");
           say(HIW"¤Ñ¤s³·"NOR"»¡¹D¡G"HIG"¹ï¤£¦í¡A¤W­±¬O¥»¬£ªº­«¦a¡C\n"NOR);
        return 1;
}
varargs int receive_damage(string type, int damage, object who)
{
        ...
        if( objectp(who) && type=="hp") set_temp("last_damage_from", who);
                ...
                if(objectp(who) )
                        ...
                        ::receive_damage(type,damage,who);
 if(query("hp")<query("max_hp")/4 && !is_busy() && !query_temp("flee") && random(100) > 70 ) 
           {
           command("hehe");
           this_object()->set_temp("flee");
           command("bye");
  message_vision(HIM "\n$N²M¯º¤@Án¡A¦^¨­­¸¤W«ÎÃ²»¡¹D¡G¦¹¦¸Áö±Ñ¡A¤U¦¸©w¬O¨ú§Aª¯©R®É¡A°ª¤s¬y¤ô...¦A·|°Õ~~«¢~~«¢~~¡C\n" NOR,this_object());
