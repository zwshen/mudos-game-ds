...
void create()
{
        set_name( "斧衛", ({"axe soldier", "axe", "soldier"}) );
        set("long", @LONG
...
LONG);
        ...
        set("str",65+random(20));
        set("dex",30);
        set("Merits/bar",6+random(2));  
        set("Merits/wit",2+random(2)); 
        set_temp("apply/damage",75+random(30));  
        set_temp("apply/armor",55+random(30)); 
        set_temp("apply/hit",55+random(30));
        set_skill("dodge", 70+random(10));
        set_skill("parry", 70+random(20));
        set_skill("unarmed", 70+random(30));
        set_skill("axe", 70+random(20));
        set_skill("berserk",40+random(51));
        set("chat_chance",8);
        set("chat_msg", ({
         (: random_move :),
           }));
        ...
        setup(); 
        set_living_name("Ten-mount_soldier");        carry_object(__DIR__"wp/big_axe.c")->wield();
...
int special_attack(object me, object target, int hitchance)
{
  ...
  wpsk = wp->query("skill_type");

  addbar = tbar - mybar + 1;
  damage =  mysk * (myint/20) + me->query_skill(wpsk);
