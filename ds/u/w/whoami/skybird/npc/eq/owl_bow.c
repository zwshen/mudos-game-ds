...
void create()
{
        set_name(HIW+BLK"梟形十字弓"NOR,({"owl crossbow","crossbow"}));
        set("long","這是一把模仿梟飛行時形態做出來的十字弓, 弓箭能用來對付遠方的對手\n"
                   "而前端的尖刺則是近距離攻擊的利器‧\n");
        if( clonep() )
        ...
        set("max_load",30);              //最大裝填量
        init_gun(35);   // 肉搏傷害力=set("weapon_prop/damage", 14);
        setup();
}
