...
void create()
{
        set_name("神農集", ({ "medicine book","book" }) );
        if( clonep() )
                set_default_object(__FILE__);
        else
        {
                set("unit", "本");
                set("long", "一本專供高階學者學習(learn)的藥典。");
		set("no_give", 1);
        }
        setup();
}
...
int do_read(string arg)
{
        ...
        if( me->query_skill("regs-knowledge") > 80 )
                return notify_fail(CYN"你已經可以掌握這本書的內容了。\n"NOR);
	if( me->query_skill("regs-knowledge") < 59 )
...
void read_book(object me, object obj, int flag)
{
        ...
        if( flag == 1 )
        {
                message_vision(HIG"$N鬆鬆衣帶，就地坐下讀起神農集。\n"NOR, me);
        ...
        {
                message_vision(HIG"$N正聚精會神的讀著神農集。\n"NOR, me);
                me->receive_damage("mp",10); 
        ...
        {
                message_vision(HIG"$N合起神農集，默想剛才讀到的新知。\n"NOR, me);
