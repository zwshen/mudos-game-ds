...
LONG
 ...
 set("Merits/bar",2+random(6));  
 set("Merits/wit",2+random(6));  
 set("Merits/tec",2+random(6));  
 set("Merits/bio",2+random(6));  
...
set_skill("hammer",20);  
 ...
 set_temp("pill",6);
 setup();
        carry_object(__DIR__"wp/horsetail_whisk")->wield();
        carry_object(__DIR__"obj/evil_book");
...
void heal()
{
    ...
    me->receive_heal("hp",me->query("max_hp/2"));
...
void die()
{
     ...
     call_out("wakeevil",5);
        ...
        HIM"\t【靈山警鐘】\n"NOR
        CYN"\n\t\t魔欲經保護者正一法師死於邪徒"+ob->query("name")+"之手！！\n"NOR
        ,users() );
