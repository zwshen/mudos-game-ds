...
int exert(object me,object target, string arg)
{
        ...
        if(arg=="off") 
        {
                if(me->query_skill_mapped("force")=="bagi_force")
                {
                	...
                	me->map_skill("force");
                	return 1;
                } else return notify_fail("你並沒有修練「八極內功\」。\n");    
        }

        if(!arg)
        {
	        if( me->query_skill_mapped("force")=="bagi_force" ) return notify_fail("你已正在修煉八極內功\了。\n");
        	if(me->query("hp") < 8 || me->query("ap") < 8 ) return notify_fail("你目前的身體狀況不好，無法修煉八極內功\。\n");
        	if(me->is_fighting() ) return notify_fail("你正忙著打架，沒空修煉八極內功\。\n");
...
//療傷功能，扣ap 補 hp ，引用評比 醫療 1
        if(arg =="heal" )
        {
            ...
            if(me->query("ap") < heal/3 ) return notify_fail("你目前的身體狀況，不能使用療傷功\能。\n");
