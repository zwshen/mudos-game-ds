...
void do_combat_attack() //戰鬥時使同一格玩家燒傷
{
  object *inv,me,z;
  int i,damage;
  me=this_object();
  ...
  for(i=0;i<sizeof(inv);i++)
  {
    ...
    if( living(inv[i]) && userp(inv[i]) )
    {
      ...
      if( damage > 0 )
      {
        message_vision(HIR"$N被高溫火燄燒個正著。"NOR,inv[i]);
...
int special_attack(object me, object target, int hitchance)
{
  ...
  if( !random(6) && !me->query_temp("weapon") ) //沒武器時製造炎之劍
  {
    me->start_busy(2);
  message_vision("\n$N左手凝聚"HIR"熔炎烈燄"NOR"，形成一把亮紅的"HIR"炎之劍"NOR"。！！\n"NOR,me);
    new(__DIR__"wp/flame-sword")->move(me);
    return 1;
  }
  else if(!random(8))
  {
    me->start_busy(2);
    message_vision("\n$N右手凝聚"HIR"熔炎烈燄"NOR"，形成一條約十碼長的"HIR"火燄鞭"NOR"。！！\n"NOR,me);
    call_out("do_super",2,me);
    return 1;
  }
  else if(!random(8)) //製造烈燄彈
  {
    me->start_busy(2);
    message_vision(HIR"\n$N狂吼一聲，張口吐出"+ball+"顆烈燄彈。！！\n"NOR,me);
    for(j=0;j<ball;j++)
    {
      new(__DIR__"obj/fire-ball")->move(environment(me));
    }
    return 1;
  }
  return 0;
