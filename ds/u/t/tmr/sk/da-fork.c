...
void delay2(object me)
{
	int hs,dt,hslv,hst,damage,temp,c,damage1,k;
	...
	for (dt=0;dt<sizeof(enemy) && dt<k;dt++)
	{
	  ...
	  if(random(me->query_skill("da-fork")*2) > random(20) )
	  {
		...
		if(damage<20) damage=20+random(20);
                    if(random(me->query_dex()*5+2)+3<random(target->query_dex()+target->query_skill("dodge")))
                		message_vision(HIG "\n可是被$N避開了！！\n"NOR,target);
                	else
                	{

        			damage = (damage1/4)+(random(damage1/6))+random(me->query_damage()/3)-c/2-random(c/2);
                		damage = COMBAT_D->Merits_damage(me,target,damage,"bar")+1;
	...
	me->receive_damage("ap",(hslv*15*(sizeof(enemy))));
	temp=10+(hslv*4*(sizeof(enemy)+1));
	if(me->query("hp")>temp) me->receive_damage("hp",10+(hslv*4*(sizeof(enemy)+1)));
	else me->set("hp",1);
	   //me->start_busy(2);
