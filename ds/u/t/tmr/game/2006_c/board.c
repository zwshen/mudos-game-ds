...
{
        seteuid(getuid()); 
        set_name( HIR "獵殺聖誕老人排行榜" NOR,({"record board","board"}) );
        set("long"," ");
        set_weight(200000);
        if(clonep() )
...
string query_save_file() { return DATA_DIR + "game/2006_Christmas"; }
...
string do_list()
{
        ...
        while( i-- )
        {
                ...
                if( game_record[ppl[i]] > 0 ) {
                            msg += sprintf( "第%4s名 ---- %13s 目前獵殺積分 %-25s 人斬",
                                chinese_number(j),ppl[i],chinese_number(game_record[ppl[i]]) );
                }

                  msg += "\n" NOR;
        }       
        msg += HIW"\n--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--==--\n"NOR;
        msg += "請打「help 聖誕節」了解活動內容\n";
