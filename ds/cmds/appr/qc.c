	...
	} else
	{
	 ...
	 if(obj->query("armor_type"))
	 {
	  ...
	  output+= sprintf("重量: %-5d\t價格: %-5d\t單位: %s\t體積: %-5d\t耐用: %-5d\n",
	  		obj->weight(),obj->query("value"),obj->query("unit"),obj->query("volume"),obj->query_temp("durable"));
	 ...
	 {
	  output=sprintf(
	  	"%s%s武器(%s)  材質:(%s[%s])   重量: %d    傷害: %d   價格: %d%s\n"+
	  	"=============================== 材質資訊 ================================\n"+
	  	"密度: %-4d  硬度: %-4d  耐用: %-4d  銳利: %-4d  價格: %-8d\n"+
	  	"=========================================================================\n"
	  	,output
	  	,HIC
	  	,obj->query("skill_type")
	  	,material
	  	,obj->M_chinese(material)
	  	,obj->weight()
	  	,obj->query("weapon_prop/damage")
	  	,obj->query("value")
	  	,NOR
		,obj->M_density(material)
		,obj->M_hardness(material)
	  	,obj->M_durable(material)
	  	,obj->M_sharp(material)
	  	,obj->M_prize(material)
	  	);

	  output+= sprintf("重量: %-5d   價格: %-5d   單位: %-5s   體積: %-5d   耐用: %-5d\n",
	  		obj->weight(),obj->query("value"),obj->query("unit"),obj->query("volume"),obj->query_temp("durable"));
	  if(obj->query("female_only") ) output += "女性專用\n";
	  if(obj->query("limit_lv")) output+=sprintf("限制等級: %d \n",obj->query("limit_lv"));
	  if(obj->query("limit_str")) output+=sprintf("限制力量: %d \n",obj->query("limit_str"));
