...
int main(object me, string arg)
{
	...
	int wound=0,check=0,i=0,silent=0;
	...
	if( stringp(dest) )
	{
	        ...
	        if(obj->is_house())
	        {
	        	...
	        	if(env->is_house()) env->set("exits/"+arg,obj);
	        }
	}
    ...
    if( objectp(me) && me->move(obj) )
    {
        if(objectp(me)) me->remove_all_enemy();
        if(check) {
           min+=me->name()+"抱著腳, 一副很痛苦的樣子。\n";
           write("\n你雙腳的傷口隱隱作痛。\n");
        }
         if(!silent && objectp(me) && objectp(environment(me)))
        {
            message( "vision", who + min, environment(me), ({me}) );
            all_inventory(env)->follow_me(me, arg);
        }
        if(me) me->delete_temp("visible_ob");
