...
int do_accept(string arg
        ...
        els
                ...
                ob = present(arg, environment(me))
                if (!ob) return noti                if(sizeof(clubs)>0)layer_club(getuid(ob));½Ð¤JÀ°(joinclub).\n");tify_fail("³o¸Ì¦³³o­Ó¤H¶Ü¡S\n")
...
int do_subleader(string arg)
{
        ...
        else
        {
                ...
                {
                        if(ob_info["level"] != 1)
                                return notify_fail("¥u¦³¤@¯ë±Ð®{¥i¥H¦¨¬°°ó¥D¡C\n");
...
int do_handover(string arg
        ...
        els
                ...
                me_info=CLUB_D->query_member_info(CLUB_ID,ge                        CLUB_D->set_member(CLUB_ID,getuid(me),me_info);tuid(ob))
                if(sizeof(ob_info)
