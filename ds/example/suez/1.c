// 老實說, 寫的很複雜QQ, 你可能要找其他人嚕:p, 我不常來這:p, sorry
#include <ansi.h>
inherit NPC;
inherit F_UNIQUE;
void start();
...
{
 if(arg=="boy") return tell_object(this_player(),"因為功\力不夠寫出對應此行動的程式，所以請不要用這種方式來解決此Quest。\n");
}
int do_say(object me, string arg)
{
...
void die()
{
        ...
        mixed *m=me->query("sarsass_2");
   sarsass = m;
 if(explode(base_name(environment(this_object()), "/")[3] == "shop")) destruct(this_object());
else {
