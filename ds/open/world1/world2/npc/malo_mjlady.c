...
string List ="";
string LookP ="";
string *NowPlayerId = ({ "","","","" }); //NowPlayerId[0] = ²ø®a
string LastDumpId="";
string LastDump="";
...
void create()
{
	set_name("§dªü«¼",({"aunt wu","aunt","wu"}));
	set("long",@LONG

¤@­Ó­D­Dªºªü«¼¥¿¦b§âª±³Â±N¡C
...
LONG
        ...
        set("unit","¦ì");
        set("talk_reply","Ãö©ó³Â±N(mj)¡Bª±ªk(play)©M³W«h(rule)ªº°ÝÃD¥i¥H°Ý(ask)§Ú¡C");
        set("inquiry",([
           "mj": (: to_say_play :),
           "³Â±N": (: to_say_play :),
           "ª±ªk": (: to_say_play :),
        	...
        	switch(me->money_type())
        	{
        		case 1:
        			if(me->query("bank/past")<BetAmount*5) {
        				message_vision(this_object()->name()+
        				"¹ïµÛ"+me->name(1)+"»¡¡G §Aªº±b¤á¸Ì­±¨S¦³¨¬°÷ªº½ä¸ê¡C\n",me);
        				return 1;
        			}
        			break;	
        		case 2:
...
//        command("say "+me->name()+"¥´¥X¤F¤@±i.."+do_check_num(str,0));
        ...
        if (count==(end-16-Goned)){
           Playing=0;
           Count++;
           message_vision(YEL+"$N»¡¡G¦¹§½"+HIC+"¬y§½"+NOR+YEL+"½Ð¦A­«·s¶}©l "+HIR+"start"+NOR+YEL+" ¡C\n"+NOR,this_object());
...
varargs int do_win(string str,object me)
{
        ...
        if (Touched&&NowPlayerId[NowPlayer]==me->query("id"))
           message_vision("$N»¡¹D¡G­J®a"+me->name()+HIG+"¦ÛºN"+NOR+"¡ã¡ã¡ã¡ã¡ã¡ã¡ã"+HIR+"¦Y¬õ"+NOR+"¡ã¡ã¡ã¡ã¡ã\n",this_object());
        	...
        	//return notify_fail("§A­n­J§A¦Û¤v¥´ªºµP??\n");
        	command("say §A­n­J§A¦Û¤v¥´ªºµP??");
        	return 1;
        }
        ...
        if (i){
           ...
           if (NowPlayerId[NowPlayer]==who["Id"]) Mj+="­J®aªáµP¦ÛºN\n";
           else{
...
/////////////
	                ...
	                switch(met->money_type())
	        	{
					...
					money = met->query("bank/past");
	        			if(money<(j+3+b)*BetAmount) {
	        				message_vision("$N¦]¬°¥I¤£¥X¿ú¨Ó«Ü¤£¦n·N«äªº¬ÝµÛ$n¶Ì¯º¡C\n",met,me);
	        			}
	        			else {
	        				met->add("bank/past",(-BetAmount*(j+3+b)));
	        			        me->add("bank/past",(BetAmount*(j+3+b)));
	        			        message_vision("$N«Ü¤£ªA®ðªºÂà±b"+BetAmount*(j+3+b)+"µ¹$n¡C\n",met,me);
	        			     }
	        			break;	
	        		case 2:
		 ...
		 score = ({ 0,1,0,j+3+b,0}); // ({ ¦ÛºN,­JµP,©ñºj,¥x¼Æ<©ñºj¬°­tªº>,¥Ç³W })
                 set_mjdata(me,score);
           ...
           { //¦ÛºN
              if (Play>=RECORD_PLAYERS )
              {
              	 CHANNEL_D->do_channel( this_object(), "game", sprintf("®¥³ß¶W¯Åª±®a%s¦ÛºN°Õ!! ¿WÄ¹ %d ¥x!!\n",
					me->name_id(1), (j+3)*(Play-1)+b));
		 score = ({ 1,0,0,(j+3)*(Play-1)+b,0 });  // ({ ¦ÛºN,­JµP,©ñºj,¥x¼Æ<©ñºj¬°­tªº>,¥Ç³W })
                 set_mjdata(me,score);
                 for(i=0;i<Play;i++)
                 {
                     if(NowPlayerId[i]==me->query("id")) continue;
			...
			score = ({ 0,0,0,-j-3-bb,0 });  // ({ ¦ÛºN,­JµP,©ñºj,¥x¼Æ<¿é¬°­tªº>,¥Ç³W })
                        set_mjdata(met,score);
					...
					money = met->query("bank/past");
	        			if(money<(j+3+bb)*BetAmount) {
	        				...
	        				met->add("bank/past",(-BetAmount*(j+3+bb)));
	        			        ...
	        			        message_vision("$N«Ü¤£ªA®ðªºÂà±b"+BetAmount*(j+3+bb)+"µ¹$n¡C\n",met,me);
	        			     }
	        			break;	
	        		case 2:
					money = met->query("bank/now");
         ...
         if(NowPlayerId[0]==who["Id"]) Count++; //³s²ø
         else
         {
           Times++;
           Count=0;
...
/////////////
	      ...
	      if(BetAmount>0) switch(me->money_type())
	        	{
					...
					money = me->query("bank/past");
        				me->add("bank/past",(-BetAmount*j));
        			        message_vision("$n¨S¦¬¤F$N"+BetAmount*j+"ªº¦s´Ú¡C\n",me,this_object());
	        			break;	
	        		case 2:
        ...
        if(Play >= RECORD_PLAYERS)
        {
	        ...
	        for (i=0; i<33; i++) Mj+="¢i";
	        Mj+="[1;36;46m¢«[0m\n";
	        Mj+="[1;36;46m¢i[0;36m¢j    "+HIW+"­^[0m¤å¦W¦r  [1m¤w[0mª±§½¼Æ  [1mÁ`[0m¾ÔÂÝ  [1m¦Û[0mºN¦¸¼Æ  [1m©ñ[0mºj¦¸¼Æ  [1m¥Ç[0m³W¦¸¼Æ   Æ[1;36;46m¢j[0m\n";
	        for (i=0;i<Play;i++){
...
int do_restart(string str)
{
        ...
        message_vision(YEL+"$N»¡¡G$n±NµP§½­«·s³]©w, ½Ð¥Î"+HIR+" addin "+NOR+YEL+"¥[¤JµP§½¡C\n"+NOR,this_object(),this_player());
        NowPlayerId[0]="";
        NowPlayerId[1]="";
        NowPlayerId[2]="";
        NowPlayerId[3]="";
...
//        for (i=0;i<sizeof(X);i++) printf("%s=%d ",X[i],MjE_Data[X[i]]);
        ...
        i%=Play;
        i++;
        message_vision("["+str+chinese_number(i)+"]µP§½¶}©l:\n²ø®a "+HIY+NowPlayerId[0]+" "+HIR+countstr+NOR+"\nÂY¥X»ë¤l"+
...
int do_setmj(string str)
{
        ...
        if (str=="tin"){
           ...
           if(member_array(numstr,Mjlist)==-1) return notify_fail("¨S¦³["+numstr+"]³oºØµP¡C\n");
           ...
           message_vision("\n$N¤j¥s¤@Án¡G"+HIW+" ¡ã §Ú ¡ã Å¥ ¡ã µP ¡ã °Õ ¡ã¡ã"+NOR+"\n\n",me);
           Auto=1;
           who["Auto"]="auto";
           return 1;
        }
...
void check_flower(mapping who)
{
        ...
        while(i--){
           ...
           if (temp=="f1"||temp=="f2"||temp=="f3"||temp=="f4"||
              ...
              if (Playing){
                 tell_room(environment(this_object()),YEL+this_player()->name()+"»¡¡G"+HIC+"ªáµP"+NOR+YEL+"¸Éªá!!¡C\n\n"+NOR+this_player()->name()+"©¹«á­±¸É¤F¤@±iµP¡C\n",this_player());
                 tell_object(this_player(),"§A»¡¡G"+HIC+"ªáµP"+NOR+"¸Éªá!!¡C\n\n§A©¹«á­±¸É¤F¤@±iµP¡C\n");
...
int do_look(string arg)
{
 	...
 	verb = query_verb();
        ...
        if (me->query("id")==P1Data["Id"]) who=P1Data;
              ...
              i=5;
              Mj+="ªF®a:"+NowPlayerId[0]+"\t «n®a:"+NowPlayerId[1]+"\t ¦è®a:"+NowPlayerId[2]+"\t ¥_®a:"+NowPlayerId[3]+"\n";
           ...
           Mj+="§Aªº³]©w¦³:";
              ...
              Mj+=show_mj(P4Data["Dump"],2)+"\n";
                 ...
                 if (Playing){
                    ...
                    Mj += "²ø®a: "+NowPlayerId[0];
                    if (Count) Mj+= "[³s²ø"+chinese_number(Count)+"]\n";
                    else Mj+="\n";
                    ...
                    Mj+="ªF®a["+NowPlayerId[0]+"]:"+NOR+p1o+"\n";
                    if(NowPlayer==1) Mj+=HIY;
                    Mj+="«n®a["+NowPlayerId[1]+"]:"+NOR+p2o+"\n";
                    if(NowPlayer==2) Mj+=HIY;
                    Mj+="¦è®a["+NowPlayerId[2]+"]:"+NOR+p3o+"\n";
                    if(NowPlayer==3) Mj+=HIY;
                    Mj+="¥_®a["+NowPlayerId[3]+"]:"+NOR+p4o+"\n";
              
                    Mj+=sprintf("[%10s]¥´¹LªºµP¦³:",P1Data["Id"]);
                    Mj+=show_mj(P1Data["Dump"],2)+"\n";

                    Mj+=sprintf("[%10s]¥´¹LªºµP¦³:",P2Data["Id"]);
                    Mj+=show_mj(P2Data["Dump"],2)+"\n";

                    Mj+=sprintf("[%10s]¥´¹LªºµP¦³:",P3Data["Id"]);
                    Mj+=show_mj(P3Data["Dump"],2)+"\n";

                    Mj+=sprintf("[%10s]¥´¹LªºµP¦³:",P4Data["Id"]);
                    Mj+=show_mj(P4Data["Dump"],2)+"\n";
                 }
                 else if (NowPlayerId[0]!="") Mj=LookP+"¥¿¦bµ¥¤H¥´µP";
                 else Mj=LookP+"¤@¦ì¥i·Rªº³Â±N¦è¬I¥¿µ¥¤H¥[¤J(addin)µP§½¡C";
        }else if (arg=="end"){
...
*/
	...
	out=find_tin(temp);
        if(out=="ERROR") write("¬Ý¤£À´§A³o¶Ã¤C¤KÁVªºµP¡C\n");
        else if(out!="")
        {
        	//out=show_mj(out,3);
        	write("¥i¥HÅ¥ªºµP¦³:["+out+"]\n");
...
return @HELP
    ...
    ¢x  bet       bet 100 ³]©w¨C¥x¬°100³f¹ô                  ³]©w½äª` ¢x
    ¢x  addin     ----------                                 ¥[¤J±Æ§½ ¢x
    ...
    ¢x  showmj    showmj <id> Åý¨ä¥L¤H¬Ý¦Û¤vªºµP             ²{µP     ¢x
    ¢x  top       top                                        ¬d¸ê®Æ   ¢x
...
int show_top(string arg)
{
	...
	if(!arg)
	{
		...
		output  = "±Æ ¦W        ª±    ®a           §½¼Æ ¦ÛºN ­JµP ©ñºj ¾ÔÁZ¤À ¥Ç³W\n";
		output += "===== ========================= ==== ==== ==== ==== ====== ====\n";
...
int do_bet(string arg)
{
	int bet_amount;
        if(Playing){
		if(!arg) {
			if(BetAmount<=0) command("say ¥Ø«e³o³õ¤ñÁÉ¨Ã¥¼³]©w½äª`¡A¯Â¤Á½RµP§Þ¡C");
        		else command("say ¥Ø«eªº½äª`¬O¨C¥x "+price_string(BetAmount,this_player()->money_type())+"¡C");
		}
		else {
           		command("say µP§½¶}©l¤§«á´N¤£¯à´«½äª`ª÷ÃB¤F¡C");
           	}
           return 1;
        }
        if(!arg){
        	if(BetAmount<=0) command("say ¥Ø«e¨Ã¥¼³]©w½äª`¡A¯Â½m¥\\¤Ò¡C");
        	else command("say ¥ý«e³]©wªº½äª`¬O¨C¥x "+price_string(BetAmount,this_player()->money_type())+"¡C");
        	return 1;
        }
	if(sscanf(arg,"%d",bet_amount)!=1) {
		write("³]©w½äª`¤èªk¬°: bet <ª÷ÃB> , ­Y¯Â®T¼Ö½Ð¥Î bet 0\n");
		return 1;
	}
	if(bet_amount < BASIC_BET ){
		command("say ½äª`¤£¥i¥H¤p©ó"+price_string(BASIC_BET,this_player()->money_type())+"¡C");
		return 1;
	}
	if(bet_amount > BET_LIMIT ){
		command("say ½äª`¤£¥i¥H¤j©ó"+price_string(BET_LIMIT,this_player()->money_type())+"¡C");
		return 1;
	}
	if(Play>0) {
		command("say ­n³]©w½äª`¥²¶·¦bÁÙ¨S¦³¤H¥[¤JµP®à(addin)«e³]©w¡C");
		return 1;
	}
	BetAmount = bet_amount;
        message_vision("$N½Ð$n±N¥»§½ªº½äª`³]©w¬°¨C¥x "+price_string(BetAmount,this_player()->money_type())+"¡C\n",this_player(),this_object());
        return 1;
}
...
string *do_sort_record(string type,string *keys)
{
	...
	while(i<sizeof(keys) )
	{
		max_value=-99999;
		mark=0;
		for(j=0;j<sizeof(temp_keys);j++) //§ä³Ì¤j­È
		{
...
int do_showmj(string str)
{
	int i;
        ...
        if(!target=present(str, environment(me))){
        	write("³o¸Ì¨S¦³³o­Ó¤H¥i¥HÀ°§A¬ÝµP¡C\n");
        	return 1;
        }
        if(target == me) {
        	write("¦Û¤v­n¬Ý¦Û¤vªºµP¥u­n«ö . ´N¥i¥H¤F¡C\n");
        	return 1;
        }
        myname=me->name(1);
        if (who["OutGon"]!=""){
        ...
        if (who["Show"]=="¥N½X")    Mj+="Åã¥Ü¤è¦¡¬°¥N½X¤è¦¡¡C";
        if (who["Show"]=="color")   Mj+="Åã¥Ü¤è¦¡¬°±m¦â¡C";
        if (who["Auto"]=="auto")    Mj+=myname+"¤w¸g³]©w¦Û°ÊÅ¥µP¡C";
        if (who["AutoTin"]==""&&who["AutoGon"]==""&&who["AutoPon"]==""&&who["Show"]==""&&who["Auto"]=="") Mj+="§A¨S¦³¥ô¦ó³]©w¡C\n";
	message_vision("$N§â¤â¤¤ªºµP®³µ¹$n¬Ý¤F¤@¤U¡C\n",me,target);
...
//		message_vision("temp=["+temp+"] mymj=["+mymj+"]\n",this_object());
		...
		if(tempnum<1 || tempnum >34) //µL·N¸qªºµP
		{
			return "";
		}
		else if(tempnum>=28 && tempnum <=34) //¦rµP
		{
...
//                    myid
	...
	out += "               ¢x  ùùùùùù  ¢x\n";
	out += "               ¢xùø      ùø¢x\n";
	out += sprintf("  %13s¢xùø  ¡î  ùø¢x%-13s\n",up_mark+up,down_mark+down);
	out += "               ¢xùø      ùø¢x\n";
	out += "               ¢x  ùùùùùù  ¢x\n";
	out += "               ¢|¢w¢w¢w¢w¢w¢}\n";
	out += sprintf("             %13s\n\n",myid_mark+myid);
