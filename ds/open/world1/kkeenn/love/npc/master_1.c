...
//        if(this_object()->is_busy() ) return;
        target = select_opponent();     // ¶Ã¼Æ¿ï¤@­Ó¼Ä¤H
        if(!target) return;
        dam = query_damage() + random(query_damage());   
        message_vision(
                HIW"\n$N"HIW"±NÃ@¤l¥æ¦Ü¥ª¤â¡A¥k¤â®º³Z¹B«l¬I®i¥X¦Û³Ðµ´©Û¡i"HIR"µ´±¡·À¹D"HIW"¡j¡I¡I\n"
                HIW"±j¤j¤S¥Rº¡¶Ë´d®ð®§ªº®ð«lªï­±¦V$n"HIW"®u±²¦Ó¥h¡A$n"HIW"¦b±¹¤â¤£¤Îªº±¡ªp¤§¤Uª½±µ©R¤¤¡I¡I\n"
                NOR RED"$n"RED"Áy¦â¦³¦p¦º¦Ç¯ëªººG¥Õ¡A¬Ý¨Ó¤w¸g¨ü¨ì¤Q¤ÀÄY­«ªº¤º¶Ë("+dam+")¡I\n\n"NOR,
                this_object(), target);
        target->add_busy(1);
            target->receive_damage("hp", dam);
}
