...
void heart_beat()
{
	...
	while( i-- )
	{
		if( !ppl[i]->query_temp("fly_speed") || !ppl[i]->query_temp("fly_long") )
			continue;
		...
		if( ppl[i]->query_temp("fly_long") > ppl[i]->query_temp("fly_speed") )
		{
			ppl[i]->add_temp("fly_long", -ppl[i]->query_temp("fly_speed"));
                        ppl[i]->add_temp("fly_time",(float)101.00/(float)100.00);
		}
		else
		{	
			j=100*ppl[i]->query_temp("fly_long")/ppl[i]->query_temp("fly_speed");
			ppl[i]->add_temp("fly_time",j/(float)100);
			ppl[i]->delete_temp("fly_speed");
			ppl[i]->delete_temp("fly_long");
                        ppl[i]->move("/open/moon");
		}
	}
}
