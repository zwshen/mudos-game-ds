...
int do_move(string arg)
{
          ...
          if(arg!="n" && arg!="s" && arg!="e" && arg!="w" 
            && arg!="ne" && arg!="se" && arg!="sw" && arg!="nw" && arg!="off")
          {
            notify_fail("沒有這個方向哦。\n");
            return 0;
          }
            ...
            if(arg == "n")
            {
              ...
              me->move(__DIR__"may24");
            }
            if(arg == "e")
            {
              message_vision("$N踏著梅花步伐，緩緩的往東邊過去。\n",me);
              me->move(__DIR__"may20");
            }
            if(arg == "s")
            {
              message_vision("$N踏著梅花步伐，緩緩的往南邊過去。\n",me);
              me->move(__DIR__"may14");
            }
            if(arg == "w")
            {
              message_vision("$N踏著梅花步伐，緩緩的往西邊過去。\n",me);
              me->move(__DIR__"may18");
            }
            if(arg == "ne")
            {
              message_vision("$N踏著梅花步伐，緩緩的往東北邊過去。\n",me);
              me->move(__DIR__"may25");
            }
            if(arg == "se")
            {
              message_vision("$N踏著梅花步伐，緩緩的往東南邊過去。\n",me);
              me->move(__DIR__"may15");
            }
            if(arg == "sw")
            {
              message_vision("$N踏著梅花步伐，緩緩的往西南邊過去。\n",me);
              me->move(__DIR__"may13");
            }
            if(arg == "nw")
            {
              message_vision("$N踏著梅花步伐，緩緩的往西北邊過去。\n",me);
              me->move(__DIR__"may23");
            }
            return 1;
          }
}
