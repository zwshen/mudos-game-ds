...
void create()
{
        ...
        setup();
         carry_object(__DIR__"wp/evilsword.c")->wield();
}
...
message_vision("$N給了清禪道長一個"+ob->query("name")+"。\n",me);
      me->set_temp("GETwp/feipike",1);
      command("say 咦！！這不是恩師的寶劍嗎！！");
      say(YEL"清禪道長顫抖著聲音道：恩師寶劍從不離身的，難道～～難道～～\n"NOR);
  command("fear");
say(YEL "清禪道長說：這位" + RANK_D->query_respect(me) + "能告訴我恩師在哪嗎？\n" NOR);
                 destruct(ob);
  return 1;
}
  }
...
say(YEL"清禪道長說：這位" + RANK_D->query_respect(me) + "前幾日我叫人打了一把長槍，這長槍可是用精鋼鍊造而成，我這就送你吧。\n"NOR);
