...
void create()
{
        set("short", "小溪");
        set("long", @LONG
這裡的水流突變，快過奔馬，挾萬鈞之勢，雷霆之會，直貫下來，力
霸無倫！
LONG
        );
        set("exits", ([ /* sizeof() == 2 */
...
int room_effect(object me)
{
   ...
   {
     if(me->query("swim")>40) return notify_fail("你的泳技在這水淺的地方，已經無法進步了。\n");
...
int do_practice(string arg)
{
        ...
        if( me->query_skill("swim",1) > 40 )
                return notify_fail("在這種水域，你的泳技無法進步了。\n");
...
{
        if ( arg=="south" && arg=="north" && this_player()->query_temp("Swim") )
                this_player()->delete_temp("Swim");
        return 0;
}
