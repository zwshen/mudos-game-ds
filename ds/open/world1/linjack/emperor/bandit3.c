...
void clean()
{
 ...
 int a;
  ...
  if(!query_temp("clean") > 3  ||
     !query_temp("die/jur")    ||
     !query_temp("die/ler")    ||
     !query_temp("die/biaw")   ||
     !query_temp("die/chang") )
  return;
  ...
  {
    if( popppl = find_player(poplist[a]) )
    {
      popppl->add("popularity", 6/sizeof(poplist) );
      str += HIC+popppl->name(1)+" ";
    }
    else continue;
  }
  delete_temp("clean");
  ...
  message("world:world1:vision",HIY"\n【江湖快報】皇城東郊四大盜被 " +str+ HIY+chinese_number(a)+"人給消滅了，真是可喜可賀。\n"NOR,users());
  return;
}
