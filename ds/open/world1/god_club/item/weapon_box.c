...
void create()
{
        set_name(HIG"武器置物箱"NOR,({ "weapon box", "box" }));
        set("unit", "個");
        set("long",@long
...
long
        ...
        set_max_capacity(10000); //2003.4.16 10000->400 by ksj
        setup();
...
int do_xput(object me, object obj, object dest, int func)
{
        ...
        if( func && dest == this_object() ) {
                if( (obj->query("skill_type") || obj->is_weapon()) && objectp(tmp = load_object("/open/world1/god_club/room_wp.c")) )
                {
                        if( tmp != environment(this_object()) )
                        {
                                write("自動將 "+ obj->name(1) + "分類到武器之屋。\n");
                                other_tar=1;
                                dest = tmp;
                        }
                }
                else if( ( obj->query("armor_type") || obj->is_armor() ) && objectp(tmp = load_object("/open/world1/god_club/room_eq.c")) )
                {
                        if( tmp != environment(this_object()) )
                        {
                                write("自動將 "+ obj->name(1) + "分類到防具之屋。\n");
                                other_tar=2;
                                dest = tmp;
                        }
                }
                else if( objectp(tmp = load_object("/open/world1/god_club/room_item.c")) )
                {
                        if( tmp != environment(this_object()) )
                        {
                                write("自動將 "+ obj->name(1) + "分類到道具之屋。\n");
                                other_tar=3;
                                dest = tmp;
                        }
                }
