// ........................ 無言  wilfred@DS
// 改了錯字 by Cominging ^^ && 加上 no_drop 使玩家不能在裡面丟東西.
// 價錢重量提高 by -alickyuen@ds-
...
int do_sleep(string str)
{
  ...
  {
    message_vision(CYN"$N拿出一個$n，很快的把$n搭好了。\n"NOR,me,ob);
...
void do_heal(object me)
{
  if(me->query("hp")+115 < me->query("max_hp")) me->add("hp",random(25)+90); // 100+350 /4 by alickyuen
    else me->set("hp",me->query("max_hp"));
  if(me->query("ap")+70 < me->query("max_ap")) me->add("ap",random(12)+40); // 50 _150 /4 by alick
    else me->set("ap",me->query("max_ap"));
  if(me->query("mp")+70 < me->query("max_mp")) me->add("mp",random(12)+40); // 50,150/4 by alick
