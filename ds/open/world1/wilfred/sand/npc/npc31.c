...
LONG
  ...
  set("chat_msg", ({
"馬商稍微清理了一下地面。\n",
(: command("say 本店的馬是萬中選一，而且價錢公道！一匹 700 文錢！") :),
...
void buy2(object me,object ob)
{
  ...
  {
        tell_object(me, HIC"馬商對你說：這.. 客倌請您親自來我這領馬(draw horse)罷！\n");
        me->add_temp("horse_trader", 1);
        me->save();
        return;
  }
...
int do_draw(string arg)
{
  ...
  if( (h = me->query_temp("horse_trader") ) < 1 )
  {
    me->delete_temp("horse_trader");
    return notify_fail("馬商對你說：客倌.. 好像您並沒有馬欠著喔。\n");
  }

  if( h > 10 )
  {
    write("馬商對你說：客倌, 我這一次只能領個 10 隻, 我先幫您領出來。\n");
    h = 10;
  }
