...
如果你的結婚戒指遺失了，嘗試著跟媒婆聊聊(talk with wang)。
...
{
 object ob;
 ob = new("/open/always/marry_ring.c");
 if( me->query("marry_id") && !present("marry ring", me) )
 {
   message_vision("$N暫時放下手邊的工作, 對著$n說：你似乎少了個東西...\n", this_object(), me);
   if( ob->can_move(me) )
   {
     ob->move(me);
     message_vision("$N急急忙忙的從裡邊拿了個東西給了$n。\n", this_object(), me);
   }
   else message_vision("$N對$n說：嗯嗯嗯.. 你好像需要丟些東西我能給你戒指喔。\n", this_object(), me);
 }
 else command("smile "+ me->query("id"));
 return 1;
}
