...
int do_summon()
{
 ...
 object me = this_player();
 object ob1,ob2;
 if( !objectp(ob1=present("summon seal",me)) ) return notify_fail("你身上沒有召喚符。\n"); 
 if( !objectp(ob2=present("corpse",environment(me))) ) return notify_fail("這裡沒有祭品喔\n"); 
 if( sizeof(filter_array( children(__DIR__"npc/dark_crocodile"), (: clonep :) ) ) > 0) 
     return notify_fail("你還是先對付眼前的敵人吧。\n"); 
 if( me->is_fighting() ) return notify_fail("你還是先對付眼前的敵人吧。\n"); 
