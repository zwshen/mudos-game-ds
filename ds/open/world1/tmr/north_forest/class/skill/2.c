...
{
int x,damage,i,target_armor = target->query_armor(),me_dex = me->query_dex() * 3;
int target_dodge = target->query_skill("dodge")/3 + random(target->query_dex()/4),me_damage = me->query_damage() + me->query_skill("soul-of-magic-sword")/4;
int sum_damage = me_damage - target_armor/2 - random(target_armor/2),targetstr=(int)target->query("str")+(int)(target->query_skill("sword")/10-2);
        string msg;
        string *attack = ({
  ...
  int s_force = this_player()->query_skill("soul-of-magic-sword")/2,
   ...
   if( me->query_skill("soul-of-magic-sword") < 70 ) return notify_fail("你的靈修幻劍還不夠純熟。\n");
     ...
     if( me->query_skill_mapped("sword") == "soul-of-magic-sword" )
        {
                ...
                me_damage = me->query_damage() + me->query_skill("soul-of-magic-sword")/4;
...
//                      COMBAT_D->report_status(target);
         ...
         if(me->query_skill("soul-of-magic-sword") < 30 ) target->start_busy(1+random(2));
           if( me->query_skill("soul-of-magic-sword") > 30 && me->query_skill("soul-of-magic-sword") < 60 ) target->start_busy(2+random(2));
        if( me->query_skill("soul-of-magic-sword") > 60 && me->query_skill("soul-of-magic-sword") < 90 ) target->start_busy(3+random(2));
   if( me->query_skill("soul-of-magic-sword") >= 90 )
                        {
    ...
    me->receive_damage("ap", 20 -  me->query_skill("soul-of-magic-sword")/15);
                        me->add_busy(1); //Luky
                        return 1;
                }
                if( arg == "sky-axe" )
    ...
    if( me->query_skill("soul-of-magic-sword") < 40 ) target->start_busy(1+random(2));
 if( me->query_skill("soul-of-magic-sword") > 50 && me->query_skill("soul-of-magic-sword") < 70 ) target->start_busy(2+random(2));
 if( me->query_skill("soul-of-magic-sword") > 80 && me->query_skill("soul-of-magic-sword") < 100 ) target->start_busy(3+random(2));
  if( me->query_skill("soul-of-magic-sword") >= 100 )
                {
                target->receive_damage("hp", me->query_damage()/3, me);
                target->start_busy(3+random(2));
...
// 以下會發生cras
             ...
             targetexp=(int)target->query("combat_exp")/5000;
                ...
                targetstr=(int)target->query("str")+(int)(target->query_skill("sword")/10-2);
...
//                      if(target->query_temp("pk_die"))break;
 ...
 if( random(me->query("level")) >   (targetlev))
                        {
