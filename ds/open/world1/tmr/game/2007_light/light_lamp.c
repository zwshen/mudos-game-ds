...
int isSuccess() {
    ...
    case "art":
        return query("wish_amount")  <= oil/ART_BASE && random(100) < 90;
...
void light2(object me) {
    ...
    if( s1 || s2 ) {
        ...
        if( oils >= 10 ) {
            ...
            if( rs > 0 ) {  // 有觸媒 
                msg += sprintf(
                           HIY "\n\t無數燦爛絢麗的"HIM"煙火"HIY"從%s"HIY"中綻發出來，在天空幻化成朵朵的"HIM"蝴蝶"HIY"！！\n\n"NOR, query("name") );
