...
// 自然奇觀：龍井明月
void open_moon2() {
        "/adm/daemons/event/moon.c"->trigger_event();
}

void open_moon() {
        call_out("open_moon2", 90+random(30) );
}
void create()
{
	set("short", "步靈城中央廣場");
...
"多從西方引進的油燈，晚上就會點起來照亮這附近。\n"
  ...
  "north" : __DIR__"bnorth.c",
	...
	set("light",1);
	set("item_desc",([
  "well" : HIC"這是一口古井，在井的旁邊常常有小孩子在玩跳進去古井的遊戲, 你可以嘗試用 jump into <地點>。\n"NOR,
]));
	set("objects", ([
		__DIR__"npc/yuanlieh" : 1,
		"/open/world1/alickyuen/phoenix/asker" : 1,
]));
	set("no_clean_up", 0);
	set("outdoors","land");
	setup();
...
int do_jump(string arg)
{
	...
	if( me->is_fighting() )
        ...
        if( !arg || sscanf(arg, "into %s", a )!=1 )
                return notify_fail("指令格式： jump into <地點名> .\n");
        if( a!="well" )
...
void reset() // by alickyuen
{
	:: reset();
	find_living("phoenix_asker")->clean();
/*
	message("vision", HIY"\n2002年中秋節特別活動『射太陽』將於"U"9月20日晚上10點至9月23日晚上10點"NOR HIY"在步靈城中央廣場舉行。\n\n"NOR, users() );
*/
}
