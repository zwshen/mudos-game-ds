...
¤W¦¸­×§ï: alickyuen@ds 10-11-2001
	¼W¥[:setrecall,outdoorºØÃþ:land, forest, snow, desert, heat, universe
*/
#define HAVE_OBLIQUE_LINK 0
#define MAP_BEGIN_LINE 2
inherit ITEM;
nosave string *outdoors_type = ({"off","land","forest","snow","desert","heat","universe"});
mixed *maps;
string *icons;
string *room_short;
...
¢usetrecall <on|off>		³]©w²£¥Í°Ï°ì¬O§_¯à¨Ï¥Îrecall(¹w³]¬°on)
...
void init()
{
	if( wizardp(environment()) ) {
		...
		add_action("do_setrecall", "setrecall");
...
{
	if(!arg) return notify_fail("«Ç¥~(land,forest,snow,desert,heat,universe): setoutdoors <xx> «Ç¤º: setoutdoors off¡C\n");
...
int do_setrecall(string arg)
{
	if(!arg || (arg!="on" && arg!="off")) return notify_fail("setrecall <on|off>¡C\n");
	if(arg=="off")
	{
		recall=1;
		write("©Ð¶¡±N¦Û°Ê¥[¤J set(\"no_recall\",1); \n");
		return 1;
	}
	if(arg=="on")
	{
		recall=0;
		write("©Ð¶¡±N¤£·|²£¥Í set(\"no_recall\",1); \n");
		return 1;
	}
}
...
//	{
		...
		if(recall ) file+="\tset(\"no_recall\",1);\n";
