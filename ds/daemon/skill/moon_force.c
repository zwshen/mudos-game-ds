...
// 這個系統要讓玩家不能用rebot. 也就是不能用相同的提示訊息.
// 修改larc功能 2001/12/16 by alickyuen
...
int exert(object me,object target, string arg)
{
	...
	if( arg == "?" )
	{
        ...
        參數:
                ...
                long-arc        長月型氣勁, 將可攻擊全體,可損壞及震落敵人武器 ( 進階 )
                shadow          可以增加防禦及閃躲的 - 殘影訣 ( 進階 ) 
...
SKILL_HELP	);
	...
	if( arg=="off" )
	{
		...
		message_vision("$N閉起的兩眼慢慢睜開，內息又流回經脈中了。\n",me);
		me->map_skill("force");
		return 1;
	}
...
//			return notify_fail("你的殘月心法根本還不用自己修煉, 去找老師學吧。\n"); //廢止Idle Skill. Luky		
		...
		message_vision("$N隨地盤腳坐下，氣運百骸，聚檀中而養息。\n",me);
	...
	if( arg=="recover" )
	{
		...
		reheal = (heal / 15) + ((sk/5)*3) + random(9) + (lv/2);
                ...
                if( me->query("hp") < 0 )
                        return notify_fail("你目前於瀕死狀態, 無法恢復生命狀況！\n");
		if( me->query("ap") < 20 )
		...
		message_vision("$N一時間靜下心來，運氣點了身上各傷口旁的大穴。\n",me);

		me->improve_skill("moon_force",me->query("int")/4 + random(4));
	...
	if( arg=="moon-arc" )
	{
		...
		message_vision("\n\t$N雙手一合一分，隨空虛劃凝出一片"HIC"「弧月」"NOR"。\n"
		HIW "\t弧月似有生命一般，以快捷無倫的疾速攻向$n"HIW"拿武器的雙手！！\n" NOR,me,target);

		me->receive_damage("hp",25);
		me->receive_damage("ap",35);
		me->start_busy(2);
        	me->improve_skill("moon_force",me->query("int")/3 + random(2));
        	me->improve_skill("force",me->query("int")/3 + random(2) );
	...
	if( arg == "long-arc" ) // wind adv 的功用: 長弧月
	{
		...
		me->improve_skill("moon_force",me->query_int()/4 + random(2));
		me->improve_skill("force",me->query_int()/4 + random(2));
		me->skill_active("moon_force", (: call_other, __FILE__, "longarc", me, tar :), random(1));
		return 1;
	}
	if( arg == "shadow" ) // wind adv 新增: 殘影訣
	{
                if( me->query_skill("sevenstar-sword") > 1)
		return notify_fail("學了昴七星劍義後就發揮不了殘影的功\力了。\n");
		if( me->query("level") < 30 )
...
void moon_shadow(object me, int ar1, int ar2)
{
    if( me->query_temp("apply/sk_dodge") <= ar1 )
        me->set_temp("apply/sk_dodge", 0);
    else me->add_temp("apply/sk_dodge", -ar1);
	...
	message_vision("$N的身形漸漸隱露了出來﹐顯然是殘影訣失去了效用！\n", me);
...
void arc(object me, object target)
{
	...
	if( !wp )
	{
         message_vision("「弧月」凌空轉了數圈，化成一陣輕煙消失了。\n",me);
		return;
	}
...
void longarc(object me, object *tar)
{
	...
	for(q=0;q<sizeof(tar);q++)
	{
		...
		&& weapon->unequip() ) {
			message_vision(HIC "「鏘」"NOR"地一聲響，一道"HIC"弧月"NOR"撞得$N脫落了手中的武器！！\n" NOR , tar[q]);
			tar[q]->start_busy(1);
                        message_vision(HIC"$N的武器好像被剛的弧月振壞了少許\！！\n"NOR, tar[q]);
			weapon->set_durable((weapon->query_durable() - random (10) ) );
...
void delay(object me)
{
	...
	{
		message_vision("$N心神如一道光劃過，瞿然停止了運功\。\n",me);
		me->delete_temp("is_busy/moon_force");
		me->map_skill("force");
		return;
	}
