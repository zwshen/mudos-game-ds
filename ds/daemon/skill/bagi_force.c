...
int exert(object me,object target, string arg)
{
    ...
    if (arg=="armor")
    {
        ...
        if (me->query_temp("bagi_force/armor") ) return notify_fail("你已經在施展「八極勁擋」。\n");
        if (me->query_temp("bagi_force/power_up")) return notify_fail("你已經在施展「八震訣」。\n");
        ...
        //                me->set_temp("bagi_force/armor", heal/6+random(heal/6));
        me->set_temp("bagi_force/armor", heal/3);
        me->add_temp("apply/armor", me->query_temp("bagi_force/armor") ); //更改armor&power_up的東東為Random-alick
        me->start_busy(2);
me->skill_active( "bagi_force",(: call_other, __FILE__, "armor", me, me->query_temp("bagi_force/armor"):), 300);
        return 1;
    }
    ...
    if (arg=="power_up")
    {
        ...
        //                me->set_temp("bagi_force/power_up", heal/4+random(heal/4));
        me->set_temp("bagi_force/power_up", heal/2);
        me->add_temp("apply/damage", me->query_temp("bagi_force/power_up") ); //更改armor&power_up的東東為Random-alick
        me->start_busy(2);
me->skill_active( "bagi_force",(: call_other, __FILE__, "power_up", me, me->query_temp("bagi_force/power_up"):), 300);
        return 1;
    }
