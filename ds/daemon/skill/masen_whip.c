...
// 加上broken-hearted的影響
// 技能每1級能有0.8%的機率能不受傷害力上限的限制
// update by Tmr 2007/07/09
...
{
        // 每1級有0.08機率，共80%
        return me->query_skill("broken-hearted")*8 >= random(1000);
}
...
int exert(object me, object target, string arg)
{
    ...
    if( me->query_skill_mapped("whip") == "masen_whip")
    {
        ...
        if( sk < 30 )
            return notify_fail("你的滅神鞭法未達到30級，無法施展「八荒六合無情鞭」。\n");
        if(me->query_skill("whip") < 50)
            return notify_fail("你的基礎鞭法未達到50級，無法施展「八荒六合無情鞭」。\n");
        if(me->query_skill("force") < 30)
            return notify_fail("你的內功\等級未達到30級，無法施展「八荒六合無情鞭」。\n");
            if(me->query("ap") < ap_cost )
                        return notify_fail("你的內勁不足，無法施展「八荒六合無情鞭」。\n");
        ...
        for(i=0;i<times;i++) {
               damage = me->query_damage() - target->query_armor()/2 - random(target->query_armor()/2);
              damage = COMBAT_D->Merits_damage(me, target,  damage, "bio");

             if( !check(me) ) {
                if( damage > 70 ) damage = 70 + random(30);
                if(weapon->id("THOUSAND_HAIR_WHIP"))
                        damage += 15;
                if(me->query_temp("BROKEN_HEARTED_DAMAGE") )
                        damage += 15;
                dmsg = "一處鞭傷("+damage+")！\n" NOR;
             } else {
                if( damage < 1 ) damage = 1;
                dmsg = "一處鞭傷("+damage+")，皮破肉爛！！\n" NOR;
                ap_cost += 5;
                delay += 1;
                }

            target_d = -COMBAT_D->dodge_factor(target,"dodge")*10/15;
            if( random(COMBAT_D->attack_factor(me, "whip")) > random(target_d) ) {

                switch(i) {
                case 0:
                    message_vision(HIR"$N"HIR"連續向$n"HIR"擊出第壹擊『"HIM"心鞭斬情絲"HIR"』，造成$n"HIR"左手"+ dmsg, me,target);
                    break;
                case 1:
                    message_vision(HIR"$N"HIR"連續向$n"HIR"擊出第貳擊『"HIR"六慾毀心神"HIR"』，造成$n"HIR"右手"+ dmsg, me,target);
                    break;
                case 2:
                    message_vision(HIR"$N"HIR"連續向$n"HIR"擊出第參擊『"HIC"七情無所生"HIR"』，造成$n"HIR"左腳"+ dmsg,me,target);
                    break;
                case 3:
                    message_vision(HIR"$N"HIR"連續向$n"HIR"擊出第肆擊『"HIB"隱心而後動"HIR"』，造成$n"HIR"右腳"+ dmsg,me,target);
                    break;
                case 4:
                    message_vision(HIR"$N"HIR"連續向$n"HIR"擊出第伍擊『"HIW"七夕銀河聚"HIR"』，造成$n"HIR"頭部"+ dmsg,me,target);
                    break;
                case 5:
                    message_vision(HIR"$N"HIR"連續向$n"HIR"擊出第陸擊『"HIY"金黃存情意"HIR"』，造成$n"HIR"腹部"+ dmsg,me,target);
                    break;
                case 6:
                    message_vision(HIR"$N"HIR"連續向$n"HIR"擊出第柒擊『"MAG"眼波傳電情"HIR"』，造成$n"HIR"臉部"+ dmsg,me,target);
                    break;
                case 7:
                    message_vision(HIR"$N"HIR"連續向$n"HIR"擊出第捌擊『"WHT"心灰喪鬼神"HIR"』，造成$n"HIR"背部"+ dmsg,me,target);
                    break;
                }
        ...
        if(12-me->query_bar() < 4)
            call_out("stop_masen", 4+delay/2, me);
        else
            call_out("stop_masen", 12-me->query_bar() + delay/2, me);
        me->start_busy(1);
        me->receive_damage("ap", ap_cost + me->query_bar()*2);

        
                // 武器損傷
        if( random(100) < 30 ) weapon->attack();
        return 1;
    }
