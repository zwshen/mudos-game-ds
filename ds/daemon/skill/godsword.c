...
int exert(object me, object target, string arg)
{
    ...
    me->reset_action();
    message_vision("\n"HIW" $N手握"+weapon->name()+"橫劍展式, 開始施展"HIR"『"NOR"神武劍訣"HIR"』"NOR"。\n"NOR,me);
...
mapping query_action(object me, object weapon)
{
    ...
    //扣ap
    if(me->query("ap") > 10 ) me->receive_damage("ap",1);

    //牽制評比 4
    if ( objectp(target) && sk > 50 && !target->is_busy() && !me->is_busy() && me->query("ap") > me->query("max_ap")*6/10 && sk > random(200))
    {

        i=COMBAT_D->attack_factor(me,"sword")*10;
        j=COMBAT_D->dodge_factor(target,"dodge")*6;
        //設定busy
        if(sk > 90 ) busy =3;
        else if( sk > 70 ) busy =2;
        else busy=1;
    ...
    if( me->query("ap") <10 )
    {
        me->map_skill("sword");
        me->reset_action();
        message_vision("\n$N神態放鬆，停止了施展"HIR"『"NOR"神武劍訣"HIR"』"NOR"。\n"NOR,me);
    }
